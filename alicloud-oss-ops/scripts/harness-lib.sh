#!/bin/bash
# SkillOpt Core Library for alicloud-oss-ops
# Self-repair and dynamic optimization for OSS CLI commands.
...
SKILLOPT_LOG_LABEL="OSS-SkillOpt"
SKILLOPT_SKILL_TAG="alicloud-oss-ops"

# Langfuse config: intentionally unset at source time so .env can populate them
# in skillopt_init. Defaults are applied by _skillopt_apply_config_defaults after .env load.
# Parent-shell exports (e.g. SKILLOPT_LANGFUSE_ENABLED=true ./wrapper.sh) still win.

# Session & Trace state
SKILLOPT_SESSION_ID="${SKILLOPT_SESSION_ID:-}"

_skillopt_load_env_file() {
    local env_file="$1"
    [[ -f "$env_file" ]] || return 0
...
_skillopt_apply_config_defaults() {
...
SKILLOPT_LOG_FILE="${ALIBABA_CLOUD_LOG_DIR:-${_SKILLOPT_LOGS_DIR:-$_SKILLOPT_RUNTIME_ROOT}}/oss-skillopt-$(date +%Y%m%d).log"
SKILLOPT_RUNTIME_DATA="${_SKILLOPT_METRICS_DATA_DIR:-$_SKILLOPT_RUNTIME_ROOT}/oss-skillopt-runtime.json"
# --- End shared core ---
