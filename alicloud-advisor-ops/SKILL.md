...
metadata:
  ...
  cli_support_evidence: >-
    ...
    RefreshAdvisorResource) have matching CLI commands. CLI is the
    primary path; SDK fallback is used only when the user needs to embed
...
aliyun advisor DescribeAdvices --product Ecs --biz-language zh
```
...
| Connectivity | `aliyun advisor GetProductList` | Non-empty product list | HALT; check network / credentials |
...
aliyun advisor DescribeAdvices --advice-id {{user.advice_id}}

# Get a single check's results
aliyun advisor DescribeAdvices --check-id {{user.check_id}
```
...
# Specific check plan
aliyun advisor DescribeCostOptimizationOverview --check-plan-id {{user.check_plan_id}
```
...
| `InvalidParameter.Product` | `Product` not in supported list | No | 0 | — | HALT; use `GetProductList` to enumerate |
