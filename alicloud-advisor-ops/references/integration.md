...
4. After fix, re-trigger Advisor scan:
   ...
   - Confirm advice gone: aliyun advisor describe-advices --check-id ...
```
...
aliyun advisor describe-advices --assume-aliyun-id 12345
```
  ...
  report:
    ...
    steps:
      ...
      - name: Generate cost report
        run: |
          aliyun advisor describe-cost-optimization-overview > cost.json
          aliyun advisor describe-cost-check-results --group-by Check >> cost.json
...
aliyun advisor refresh-advisor-resource --product Ecs --resource-id i-test
```
...
- [ ] `get-product-list` returns non-empty list
