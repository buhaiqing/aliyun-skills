...
|---------|------|---------|
| 1.6.0 | 2026-06-11 | Add `assets/scripts/slow-sql-aggregator.py`: multi-dimension slow SQL aggregation tool with auto-pagination, SQLHash dedup, and 5-section report (by count/time/node/db/minute). Update `references/advanced/slow-query-analysis.md` §2.3 with script usage. (DOPS-85809) |
...
> 
> **聚合分析脚本**: [assets/scripts/slow-sql-aggregator.py](assets/scripts/slow-sql-aggregator.py) — 全量拉取 + SQLHash 去重 + 多维度报告（按次数/总耗时/节点/数据库/时间分布），支持自动分页，零外部依赖。
...
- [Slow SQL Aggregator Script](assets/scripts/slow-sql-aggregator.py) — 慢 SQL 聚合分析脚本：全量拉取 + 多维度报告
