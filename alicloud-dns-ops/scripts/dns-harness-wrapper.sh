#!/bin/bash
# DNS Runtime Harness wrapper (preferred entry)
...
PRODUCT="alidns"
if [[ ${#} -gt 0 && ("$1" == "alidns" || "$1" == "pvtz") ]]; then
    PRODUCT="$1"
    shift
fi

if [[ "${#}" -lt 1 ]]; then
    echo "Usage: $0 [alidns|pvtz] <subcommand> [params]" >&2
    exit 1
fi
...
if [ "$SKILLOPT_LOADED" = true ]; then
    echo "[Wrapper] alicloud-dns-ops/scripts/dns-harness-wrapper.sh ${SUBCMD}" >&2
