#!/bin/bash
# SkillOpt Core Library for alicloud-redis-ops
# Self-repair and dynamic optimization for Redis CLI commands.
...
SKILLOPT_LOG_LABEL="Redis-SkillOpt"
SKILLOPT_SKILL_TAG="alicloud-redis-ops"
...
SKILLOPT_LOG_FILE="${ALIBABA_CLOUD_LOG_DIR:-${_SKILLOPT_LOGS_DIR:-$_SKILLOPT_RUNTIME_ROOT}}/redis-skillopt-$(date +%Y%m%d).log"
SKILLOPT_RUNTIME_DATA="${_SKILLOPT_METRICS_DATA_DIR:-$_SKILLOPT_RUNTIME_ROOT}/redis-skillopt-runtime.json"
# --- End shared core ---
...
skillopt_repair_error() {
    ...
    case "$error_code" in
        ...
        Forbidden|NoPermission)
            ...
            skillopt_log "HINT: Ensure AK has policy: {\"Action\":[\"redis:*\"],\"Effect\":\"Allow\",\"Resource\":\"*\"}"
            ;;
        ...
        QuotaExceeded)
            skillopt_log "Quota exceeded for Redis"
            ;;
