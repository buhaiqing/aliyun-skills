...
if [ "$DETECTED_SHELL" = "zsh" ]; then
    print_status "WARN" "zsh detected - wrap CLI output expressions in quotes to avoid glob interpretation (e.g., 'rows=Instances.KVStoreInstance[]')"
...
#=============================================================================
section "3" "CLI Plugin (aliyun-cli-r-kvstore)"

PLUGIN_DIR="$HOME/.aliyun/plugins"
PLUGIN_NAME="aliyun-cli-r-kvstore"
...
plugin_works() {
    local test_output
    test_output=$(aliyun r-kvstore describe-regions 2>&1 || true)
    ! echo "$test_output" | grep -qE "Plugin.*required.*not installed|is not a valid built-in product"
}

if is_stale_plugin_dir; then
    print_status "WARN" "Stale/empty plugin directory detected: $PLUGIN_PATH"
    print_status "INFO" "Attempting cleanup (handling macOS extended attributes)..."
    if chmod -R u+w "$PLUGIN_PATH" 2>/dev/null && \
       ...
       rm -rf "$PLUGIN_PATH" 2>/dev/null; then
        ...
        print_status "WARN" "Could not remove stale directory via shell, deferring to plugin install"
...
if plugin_works; then
    ...
    print_status "PASS" "Redis/Tair plugin installed and verified (describe-regions SUCCESS)"
else
    print_status "WARN" "Redis/Tair plugin not working or not installed"
    DETECTED_CLI_PLUGIN_INSTALLED=false
    ...
    if ! $install_ok; then
        ...
        add_issue "WARNING" "Cannot install aliyun-cli-r-kvstore plugin" "Use SDK fallback path: go run scripts/sdk-fallback.go"
    fi
fi
...
if $DETECTED_CLI_WORKING; then
    region_test=$(aliyun r-kvstore describe-instances --region "$ALIBABA_CLOUD_REGION_ID" --PageSize 1 --output cols=TotalCount rows=TotalCount 2>&1 || true)
    ...
    output_test=$(aliyun r-kvstore describe-instances --region "$ALIBABA_CLOUD_REGION_ID" --PageSize 1 --output 'cols=InstanceId rows=Instances.KVStoreInstance[0]' 2>&1 || true)
    if echo "$output_test" | grep -qE "^InstanceId" && ! echo "$output_test" | grep -qi "error"; then
        print_status "PASS" "CLI output format (cols/rows) works with quoted arguments"
    else
        print_status "PASS" "CLI output format works (JSON mode available)"
    fi
else
...
#=============================================================================
# Check 5: Credentials (with API-level validation)
#=============================================================================
...
if $creds_ok && $DETECTED_CLI_WORKING; then
    api_args=""
    [ "$DETECTED_CLI_PARAM_STYLE" = "camelcase" ] && api_args="--RegionId" || api_args="--region"
    validate_output=$(aliyun r-kvstore describe-regions $api_args "$ALIBABA_CLOUD_REGION_ID" 2>&1 || true)

    if ! echo "$validate_output" | grep -qiE "error|traceid|unknown|unable"; then
        DETECTED_CREDENTIALS_VALID=true
        print_status "PASS" "API credential validation: SUCCESS (describe-regions returned regions)"
    else
        DETECTED_CREDENTIALS_VALID=false
        print_status "FAIL" "API credential validation: FAILED (describe-regions error)"
...
#=============================================================================
# Check 6: Go Runtime (SDK Fallback)
#=============================================================================
section "6" "Go Runtime (SDK Fallback)"
...
case "$OVERALL_STATUS" in
    PASS)
        echo -e "${GREEN}✓ CLI (Primary):       aliyun r-kvstore <command> [args]${NC}"
    ...
    WARNING)
        ...
        echo -e "${YELLOW}◆ CLI (if available):         aliyun r-kvstore <command> [args]${NC}"
