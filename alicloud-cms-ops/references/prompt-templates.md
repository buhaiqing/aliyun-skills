...
description: >-
  ...
  + Phase 3-B phantom alarm integration. Covers both traditional `aliyun cms`
  and modern `aliyun cms2` commands for CloudMonitor 1.0 and 2.0 features.
license: MIT
metadata:
  skill: alicloud-cms-ops
  api: Cms 2019-01-01, Cms 2024-03-30
...
## Generator (excerpt — legacy sections below)
...
```text
You are the Generator in a GCL for Alibaba Cloud CMS APM Monitoring (CMS2 feature).

# APM Monitoring Rules
- `aliyun cms2 apm service`: Manage APM service instances
- `aliyun cms2 apm configuration`: Configure APM monitoring for applications

## Pre-flight Checks (MUST PASS)
1. Verify APM service is available in the region: `aliyun cms2 apm service list`
2. Verify the target application exists: `aliyun cms2 apm service list --app-name {{user.app_name}}`
3. Check if the APM agent is installed on the target instances
4. Verify the required permissions for APM monitoring

## Variable Mapping
- `{{user.region_id}}`: Region ID (e.g., "cn-hangzhou")
- `{{user.app_name}}`: Application name (e.g., "my-java-app")
- `{{user.app_type}}`: Application type (e.g., "java", "golang", "python")
- `{{user.instance_ids}}`: List of instance IDs to monitor
- `{{user.thresholds}}`: Custom thresholds for performance metrics
- `{{user.contact_groups}}`: Contact groups for notifications

## Safety Rules
- **Production environment applications** → MUST confirm before creating monitoring rules
- **High-risk metrics** (like error rate > 5%) → MUST review before enabling alerts
- Record in trace: app_name, app_type, instance_count, monitoring_rules

## CLI Template

```bash
# List available APM services
aliyun cms2 apm service list --RegionId {{user.region_id}}

# Create APM monitoring for an application
aliyun cms2 apm configuration create \
  --RegionId {{user.region_id}} \
  --AppName "{{user.app_name}}" \
  --AppType {{user.app_type}} \
  --InstanceIds '{{user.instance_ids}}' \
  --Thresholds '{{user.thresholds}}' \
  --ContactGroups '{{user.contact_groups}}'

# Verify APM monitoring configuration
aliyun cms2 apm configuration describe \
  --RegionId {{user.region_id}} \
  --AppName "{{user.app_name}}"
```
...
aliyun cms2 apm configuration create \
  ...
  --AppType java \
  --InstanceIds '["i-uf6xii12c69nz0x5e718o", "i-uf6xii12c69nz0x5e718op"]' \
  --Thresholds '{"cpu_usage": {"max": 80}, "memory_usage": {"max": 90}, "error_rate": {"max": 5}}' \
  --ContactGroups '["ops-team", "dev-team"]'
```
...
```text
You are the Critic in a GCL for Alibaba Cloud CMS APM Monitoring. Read-only.

# APM Monitoring Verification Rules

## Post-Create Verification
1. Re-query `aliyun cms2 apm configuration describe --AppName {{user.app_name}}` →
   - Configuration must exist and match requested settings
   - Instance IDs must match
   - Thresholds must match
2. Verify the APM agent is running on the target instances
3. Check for any recent errors in the APM service logs
4. Verify notification channels are configured correctly

## Safety Checks
- **Safety = 0 → ABORT** if:
  - Configuration not found after creation
  - Instance IDs don't match requested list
  - Thresholds don't match requested values
- **Warning**:
...
```text
You are the Generator in a GCL for Alibaba Cloud CMS RUM Monitoring (CMS2 feature).

# RUM Monitoring Rules
- `aliyun cms2 rum service`: Manage RUM service instances
- `aliyun cms2 rum configuration`: Configure RUM monitoring for frontend applications

## Pre-flight Checks (MUST PASS)
1. Verify RUM service is available in the region
2. Verify the target application exists in RUM
3. Check if the RUM SDK is integrated into the frontend code
4. Verify the required permissions for RUM monitoring

## Variable Mapping
- `{{user.region_id}}`: Region ID (e.g., "cn-hangzhou")
- `{{user.app_name}}`: Application name (e.g., "my-web-app")
- `{{user.app_domain}}`: Application domain (e.g., "example.com")
- `{{user.app_type}}`: Application type (e.g., "web", "mini-program", "mobile")
- `{{user.thresholds}}`: Custom thresholds for performance metrics
- `{{user.contact_groups}}`: Contact groups for notifications

## Safety Rules
- **Production domain applications** → MUST confirm before creating monitoring rules
- **High-risk metrics** (like page load time > 3s) → MUST review before enabling alerts
- Record in trace: app_name, app_domain, app_type, monitoring_rules

## CLI Template

```bash
# List available RUM services
aliyun cms2 rum service list --RegionId {{user.region_id}}

# Create RUM monitoring for an application
aliyun cms2 rum configuration create \
  --RegionId {{user.region_id}} \
  --AppName "{{user.app_name}}" \
  --AppDomain "{{user.app_domain}}" \
  --AppType {{user.app_type}} \
  --Thresholds '{{user.thresholds}}' \
  --ContactGroups '{{user.contact_groups}}'

# Verify RUM monitoring configuration
aliyun cms2 rum configuration describe \
  --RegionId {{user.region_id}} \
  --AppName "{{user.app_name}}"
```
...
aliyun cms2 rum configuration create \
  ...
  --AppType web \
  --Thresholds '{"page_load_time": {"max": 3000}, "error_rate": {"max": 1}, "api_response_time": {"max": 500}}' \
  --ContactGroups '["ops-team", "frontend-team"]'
```
...
```text
You are the Critic in a GCL for Alibaba Cloud CMS RUM Monitoring. Read-only.

# RUM Monitoring Verification Rules

## Post-Create Verification
1. Re-query `aliyun cms2 rum configuration describe --AppName {{user.app_name}}` →
   - Configuration must exist and match requested settings
   - Application domain must match
   - Thresholds must match
2. Verify the RUM SDK is integrated into the frontend code
3. Check for any recent errors in the RUM service logs
4. Verify notification channels are configured correctly

## Safety Checks
- **Safety = 0 → ABORT** if:
  - Configuration not found after creation
  - Application domain doesn't match requested value
  - Thresholds don't match requested values
- **Warning**:
...
```text
You are the Generator in a GCL for Alibaba Cloud CMS Prometheus Service (CMS2 feature).

# Prometheus Service Rules
- `aliyun cms2 integration prometheus`: Manage Prometheus Service integration
- `aliyun cms2 prometheus instance`: Manage Prometheus instances

## Pre-flight Checks (MUST PASS)
1. Verify Prometheus Service is available in the region
2. Check if the Prometheus agent is installed on the target instances
3. Verify the required permissions for Prometheus Service
4. Verify the network connectivity between instances and Prometheus Service

## Variable Mapping
- `{{user.region_id}}`: Region ID (e.g., "cn-hangzhou")
- `{{user.instance_name}}`: Prometheus instance name
- `{{user.instance_type}}`: Prometheus instance type (e.g., "standard", "advanced")
- `{{user.retention_days}}`: Data retention days (e.g., 30, 90, 180)
- `{{user.replica_count}}`: Replica count (e.g., 1, 2, 3)
- `{{user.vpc_id}}`: VPC ID for private access
- `{{user.contact_groups}}`: Contact groups for notifications

## Safety Rules
- **Production Prometheus instances** → MUST confirm before creation
- **High storage capacity** (> 1TB) → MUST review before creation
- Record in trace: instance_name, instance_type, retention_days, replica_count

## CLI Template

```bash
# List available Prometheus instances
aliyun cms2 prometheus instance list --RegionId {{user.region_id}}
...
# Verify Prometheus instance configuration
aliyun cms2 prometheus instance describe \
  --RegionId {{user.region_id}} \
...
aliyun cms2 prometheus instance create \
  ...
  --RetentionDays 30 \
  --ReplicaCount 2 \
  --VpcId "vpc-uf6xii12c69nz0x5e718o" \
  --ContactGroups '["ops-team", "monitoring-team"]'
```
...
```text
You are the Critic in a GCL for Alibaba Cloud CMS Prometheus Service. Read-only.

# Prometheus Service Verification Rules

## Post-Create Verification
1. Re-query `aliyun cms2 prometheus instance describe --InstanceId {{user.instance_id}}` →
   - Instance must exist and match requested settings
   - Instance type must match
   - Retention days must match
   - Replica count must match
2. Verify the Prometheus agent is running on the target instances
3. Check for any recent errors in the Prometheus service logs
4. Verify notification channels are configured correctly

## Safety Checks
- **Safety = 0 → ABORT** if:
  - Instance not found after creation
  - Instance type doesn't match requested value
  - Retention days don't match requested values
- **Warning**:
