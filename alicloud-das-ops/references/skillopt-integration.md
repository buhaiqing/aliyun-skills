# Runtime Harness Integration for alicloud-das-ops
...
### Direct Command
```bash
aliyun das DescribeInstances --skillopt-enable --RegionId cn-hangzhou
```

### Wrapper Script
```bash
cd $(pwd)/alicloud-das-ops
./scripts/das-skillopt-wrapper.sh DescribeInstances --RegionId cn-hangzhou
```
...
./scripts/das-harness-wrapper.sh GetInstance

# Check local trace
ls -lt .runtime/traces/alicloud-das-ops/ | head -3

# Verify in Langfuse UI
# Navigate to https://hai-langfuse-int.hd123.com → Sessions → skill:alicloud-das-ops
```
