# Well-Architected Assessment — Alibaba Cloud SMS Service

This document evaluates the SMS skill's operations against Alibaba Cloud's
...
|-------------|----------|
| **RAM Policy** | Use scoped permissions: `acs:dysmsapi:*:*:sms/*`. Avoid wildcard `*` actions. |
...
|-------------|---------------|---------|
| Pay-per-SMS | Low volume, variable | N/A |
| SMS Package | High volume, predictable | Up to 30% |
...
|--------|-----------|---------------------|
| DeliveryRate | acs_alicloud_dysmsapi | Optimize templates, fix phone numbers |
| SendSpeed | acs_alicloud_dysmsapi | Adjust rate limits |
| DailyUsage | acs_alicloud_dysmsapi | Scale packages, monitor quota |
