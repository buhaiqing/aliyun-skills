# Runtime Harness Integration for alicloud-sms-ops
...
6. **Quota Exceeded** (`QUOTA_EXCEEDED`) — notifies user of limits
...
./scripts/dysmsapi-harness-wrapper.sh DescribeInstances --RegionId cn-hangzhou
```
...
./scripts/sms-harness-wrapper.sh QuerySendDetails --PhoneNumbers 13800138000 --SendDate $(date +%Y-%m-%d)

# Check local trace
ls -lt .runtime/traces/alicloud-sms-ops/ | head -3

# Verify in Langfuse UI
# Navigate to https://hai-langfuse-int.hd123.com → Sessions → skill:alicloud-sms-ops
```
