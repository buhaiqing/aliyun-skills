#!/bin/bash
# SkillOpt Core Library for alicloud-eip-ops
# Self-repair and dynamic optimization for EIP CLI commands.
...
SKILLOPT_LOG_LABEL="EIP-SkillOpt"
SKILLOPT_SKILL_TAG="alicloud-eip-ops"
    ...
    case "$error_code" in
        ...
        InvalidParameter|InvalidJSON|MissingParameter|MissingParameter.RegionId|InvalidParameter.RegionId|MissingRegionId|DataRetentionExceeded|InvalidTimeRange|TimeRangeExceeded|InvalidParameter.StartTime|InvalidParameter.TimeRange)
            ...
            if $is_region_error; then
                ...
                if [[ -z "$region" ]]; then
                    if _SKILLOPT_INTERNAL_REPAIR=1 skillopt_run_aliyun vpc DescribeRegions --output cols=RegionId rows=Regions.Region 2>/dev/null; then
        ...
        InvalidAllocationId.NotFound|ResourceNotFound)
        ...
        QuotaExceeded)
            skillopt_log "Quota exceeded for EIP"
            ;;
