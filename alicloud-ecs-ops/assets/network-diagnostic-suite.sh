...
if $HAS_PUBLIC_IP; then
  echo "  Generating workload via loopback TCP..."
  # Try nc self-connect first (generates real loopback traffic)
  if command -v nc >/dev/null 2>&1; then
    (timeout 12 sh -c "nc -l 9999 & srv=\$!; sleep 1; (yes | nc localhost 9999 & cl=\$!; sleep 10; kill \$cl 2>/dev/null); kill \$srv 2>/dev/null" 2>/dev/null || true) &
    CP=$!; sleep 12; kill -0 "$CP" 2>/dev/null && kill "$CP" 2>/dev/null; wait "$CP" 2>/dev/null || true
    WLM="loopback-nc"
  else
    # Fallback: CPU-only workload to demonstrate delta sampling mechanism
    (timeout 12 dd if=/dev/zero of=/dev/null bs=1M 2>/dev/null || true) &
    CP=$!; sleep 12; kill -0 "$CP" 2>/dev/null && kill "$CP" 2>/dev/null; wait "$CP" 2>/dev/null || true
    WLM="cpu-burn"
  fi
else
