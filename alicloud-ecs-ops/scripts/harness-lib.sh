#!/bin/bash
# SkillOpt Core Library for alicloud-ecs-ops
# Self-repair and dynamic optimization for ECS CLI commands.
...
SKILLOPT_LOG_FILE="${ALIBABA_CLOUD_LOG_DIR:-${_SKILLOPT_LOGS_DIR:-$_SKILLOPT_RUNTIME_ROOT}}/ecs-skillopt-$(date +%Y%m%d).log"
SKILLOPT_RUNTIME_DATA="${_SKILLOPT_METRICS_DATA_DIR:-$_SKILLOPT_RUNTIME_ROOT}/ecs-skillopt-runtime.json"
# --- End shared core ---
    ...
    case "$error_code" in
        ...
        ResourceNotFound)
            ...
            if [[ -n "$instance_id" && -n "$region" ]]; then
                ...
                elif [[ "$instance_id" =~ ^d- ]]; then
                    check_cmd="aliyun ecs DescribeDisks --RegionId $region --DiskIds '[\"$instance_id\"]'"
                elif [[ "$instance_id" =~ ^sg- ]]; then
                    check_cmd="aliyun ecs DescribeSecurityGroups --RegionId $region --SecurityGroupIds '[\"$instance_id\"]'"
                elif [[ "$instance_id" =~ ^v- ]]; then
                    check_cmd="aliyun vpc DescribeVSwitches --RegionId $region --VSwitchId '$instance_id'"
                elif [[ "$instance_id" =~ ^sp- ]]; then
                    check_cmd="aliyun ess DescribeScalingConfigurations --RegionId $region --ScalingConfigurationId '$instance_id'"
                fi
