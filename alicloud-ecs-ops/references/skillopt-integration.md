# Runtime Harness Integration for alicloud-ecs-ops
...
### Direct Command
```bash
aliyun ecs DescribeInstances --skillopt-enable --RegionId cn-hangzhou
```

### Wrapper Script
```bash
cd $(pwd)/alicloud-ecs-ops
./scripts/ecs-skillopt-wrapper.sh DescribeInstances --RegionId cn-hangzhou
```
...
./scripts/ecs-harness-wrapper.sh DescribeInstances --RegionId cn-hangzhou

# Check local trace
ls -lt .runtime/traces/alicloud-ecs-ops/ | head -3

# Verify in Langfuse UI
# Navigate to https://hai-langfuse-int.hd123.com → Sessions → skill:alicloud-ecs-ops
```
