...
aliyun cms DescribeMetricList \
  ...
  --Period 300 --StartTime "$(date -u -d '1 hour ago' +%Y-%m-%dT%H:%M:%SZ 2>/dev/null || date -u -v-1H +%Y-%m-%dT%H:%M:%SZ 2>/dev/null)"
...
aliyun cms DescribeAlertHistoryList \
  --StartTime "$(date -u -d '10 minutes ago' +%Y-%m-%dT%H:%MZ 2>/dev/null || date -u -v-10M +%Y-%m-%dT%H:%MZ 2>/dev/null)" \
