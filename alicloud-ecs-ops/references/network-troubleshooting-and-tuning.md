...
aliyun log GetLogs \
  ...
  --from "$(date -u -d '1 hour ago' +%s 2>/dev/null || date -u -v-1H +%s 2>/dev/null)" \
  --to "$(date -u +%s)"
```
...
aliyun ecs DescribeInstanceHistoryEvents \
  --RegionId cn-hangzhou \
  --InstanceId i-xxx \
  --EventPublishTime.Start "$(date -u -d '1 hour ago' +%Y-%m-%dT%H:%M:%SZ 2>/dev/null || date -u -v-1H +%Y-%m-%dT%H:%M:%SZ 2>/dev/null)"
```
