# Runtime Harness Integration for alicloud-ask-ops
...
### Direct Command
```bash
aliyun cs DescribeInstances --skillopt-enable --RegionId cn-hangzhou
```

### Wrapper Script
```bash
cd $(pwd)/alicloud-ask-ops
./scripts/ask-skillopt-wrapper.sh DescribeInstances --RegionId cn-hangzhou
```
...
./scripts/ask-harness-wrapper.sh GET /api/v2/clusters

# Check local trace
ls -lt .runtime/traces/alicloud-ask-ops/ | head -3

# Verify in Langfuse UI
# Navigate to https://hai-langfuse-int.hd123.com → Sessions → skill:alicloud-ask-ops
```
