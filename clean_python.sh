...
PATTERNS=(
    ...
    "SkillOpt runtime logs|*-skillopt-*.log|f"
    "SkillOpt trace JSON|traces|d"
    ".runtime dirs|.runtime|d"
    "audit-results dirs|audit-results|d"
    ".terraform dirs|.terraform|d"
    "Terraform state|*.tfstate|f"
    "Terraform backup|*.tfstate.backup|f"
    "node_modules dirs|node_modules|d"
)
...
for entry in "${PATTERNS[@]}"; do
    ...
    done < <(find . $kind_flag -name "$pattern" \
