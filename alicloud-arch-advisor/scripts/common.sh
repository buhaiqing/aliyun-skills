...
# ---------------------------------------------------------------------------
source "${ARCH_ADVISOR_DIR}/scripts/i18n.sh"

# ---------------------------------------------------------------------------
# Error Handling
# ---------------------------------------------------------------------------
source "${ARCH_ADVISOR_DIR}/scripts/error-handler.sh"

# ---------------------------------------------------------------------------
# Logging (with i18n support)
# ---------------------------------------------------------------------------
log_info()  { echo -e "${BLUE}$(t 'common_info')${NC} $*" >&2; }
log_warn()  { echo -e "${YELLOW}$(t 'common_warn')${NC} $*" >&2; }
log_error() { echo -e "${RED}$(t 'common_error')${NC} $*" >&2; }
log_success() { echo -e "${GREEN}$(t 'common_success')${NC} $*" >&2; }
...
# Start progress tracking for a multi-step operation
# Usage: progress_start <total_steps> [description]
progress_start() {
    local total_steps="$1"
...
# Update progress bar
# Usage: progress_update <current_step> [step_description]
progress_update() {
    local current_step="$1"
    local step_desc="${2:-}"
    _progress_current_step=$current_step
    
    local percentage=0
    if [[ $_progress_total_steps -gt 0 ]]; then
        percentage=$(( (current_step * 100) / _progress_total_steps ))
    fi
    
    # Calculate ETA
    local elapsed=0
    local eta="--"
    if [[ $_progress_start_time -gt 0 && $current_step -gt 0 ]]; then
        local now
        now=$(date +%s)
        elapsed=$((now - _progress_start_time))
        if [[ $elapsed -gt 0 ]]; then
            local steps_per_sec
            steps_per_sec=$(echo "scale=2; $current_step / $elapsed" | bc 2>/dev/null || echo "0")
            if [[ "$steps_per_sec" != "0" ]]; then
                local remaining_steps
                remaining_steps=$((_progress_total_steps - current_step))
                local eta_seconds
                eta_seconds=$(echo "scale=0; $remaining_steps / $steps_per_sec" | bc 2>/dev/null || echo "0")
    ...
    # Output progress line (overwrite previous line)
    local time_info=""
    [[ $elapsed -gt 0 ]] && time_info=" | $(t 'progress_elapsed' "$elapsed")"
    [[ "$eta" != "--" ]] && time_info+=" | $(t 'progress_eta' "$eta")"
    
    echo -ne "\r${GREEN}▌${bar}▐ ${percentage}%${time_info}${NC}" >&2
...
# Usage: progress_complete [final_message]
progress_complete() {
    local final_msg="${1:-Complete}"
    
    # Force to 100%
    progress_update $_progress_total_steps
    
    local total_time=0
    if [[ $_progress_start_time -gt 0 ]]; then
        local now
        now=$(date +%s)
        total_time=$((now - _progress_start_time))
    fi
    
    echo -e "\n$(t 'progress_start')" >&2
    echo -e "$(t 'progress_complete' "$final_msg" "$total_time")" >&2
    echo -e "$(t 'progress_start')\n" >&2
    
    # Persist final state
    if [[ -n "${_progress_state_file:-}" ]]; then
        _progress_save_state "completed" "$final_msg" "$total_time"
    fi
...
# ---------------------------------------------------------------------------
# Nested Progress Bar Support (v1.4.0+)
# ---------------------------------------------------------------------------

# Track current nesting depth (0 = root level)
_progress_depth=0
# Stack of progress contexts (using temp files for Bash 3.2 compatibility)
_progress_stack_dir="${TMPDIR:-/tmp}/arch-advisor-progress-$$"

# Detect terminal type for graphical progress bar
detect_terminal_capability() {
    ...
    if [[ -n "${TERM_PROGRAM:-}" && "$TERM_PROGRAM" == "iTerm.app" ]]; then
        ...
        echo "256color"
    elif command -v tput &>/dev/null && [[ $(tput colors 2>/dev/null) -ge 256 ]]; then
        echo "256color"
    else
        echo "basic"
    fi
...
_TERM_CAPABILITY=$(detect_terminal_capability)
export _TERM_CAPABILITY

# Start a nested progress level (sub-task)
# Usage: progress_nested_start <total_substeps> <description>
progress_nested_start() {
    local total_substeps="$1"
    local description="$2"
    
    # Save parent context
    _progress_push_context
    
    # Increase depth
    _progress_depth=$((_progress_depth + 1))
    
    # Initialize sub-progress
    _progress_start_time=$(date +%s)
    _progress_total_steps=$total_substeps
    _progress_current_step=0
    
    local indent=""
    for ((i=0; i<_progress_depth; i++)); do indent+="  "; done
    
    echo -e "${indent}${CYAN}┌── ${description}${NC}" >&2
}

# Update nested progress (works for any depth level)
# Usage: progress_nested_update <current_step> [step_description]
progress_nested_update() {
    local current_step="$1"
    local step_desc="${2:-}"
    _progress_current_step=$current_step
    ...
    # Build mini progress bar (20 chars for nested)
    local bar_width=20
    local filled=$(( (percentage * bar_width) / 100 ))
    local empty=$((bar_width - filled))
    ...
    echo -ne "\r${indent}${GREEN}│ ${bar} ${percentage}%${NC}" >&2
    
    if [[ -n "$step_desc" ]]; then
        echo -e " ${BLUE}${step_desc}${NC}" >&2
    fi
}

# Complete nested progress level
# Usage: progress_nested_complete [final_message]
progress_nested_complete() {
    local final_msg="${1:-Done}"
    
    progress_nested_update $_progress_total_steps
    ...
    echo -e "${indent}${GREEN}└── ✓ ${final_msg} (${total_time}s)${NC}" >&2
    
    # Restore parent context
    _progress_pop_context
}

# Internal: Push current progress context to stack
_progress_push_context() {
    mkdir -p "$_progress_stack_dir"
    local ctx_file="$_progress_stack_dir/ctx_${_progress_depth}.env"
    {
        echo "_progress_start_time=${_progress_start_time}"
        echo "_progress_total_steps=${_progress_total_steps}"
        echo "_progress_current_step=${_progress_current_step}"
    } > "$ctx_file"
}

# Internal: Pop progress context from stack
_progress_pop_context() {
    local ctx_file="$_progress_stack_dir/ctx_${_progress_depth}.env"
    if [[ -f "$ctx_file" ]]; then
        source "$ctx_file"
    ...
    _progress_depth=$((_progress_depth - 1))
    if [[ $_progress_depth -lt 0 ]]; then
        _progress_depth=0
    fi
}

# ---------------------------------------------------------------------------
# Progress State Persistence (v1.4.0+)
# ---------------------------------------------------------------------------

# Save progress state to disk for resume capability
# Usage: _progress_save_state <status> <message> <elapsed>
_progress_save_state() {
    local status="${1:-running}"
    local message="${2:-}"
    local elapsed="${3:-0}"
    
    if [[ -z "${_progress_state_file:-}" ]]; then
        return 0
    fi
    
    local state_dir
    state_dir=$(dirname "$_progress_state_file")
    mkdir -p "$state_dir"
...
# Enable progress state persistence
# Usage: progress_persistence_enable <state_file_path>
progress_persistence_enable() {
    local state_file="$1"
    _progress_state_file="$state_file"
    _progress_save_state "running" "Started" 0
}

# Disable progress state persistence
progress_persistence_disable() {
    if [[ -n "${_progress_state_file:-}" && -f "$_progress_state_file" ]]; then
        rm -f "$_progress_state_file"
    fi
    unset _progress_state_file
}

# Resume from previous progress state
# Usage: progress_resume <state_file_path>
# Returns: 0 if resumed, 1 if no state found
progress_resume() {
    local state_file="$1"
    ...
    local status current_step total_steps
    status=$(jq -r '.status' "$state_file" 2>/dev/null) || return 1
    current_step=$(jq -r '.current_step' "$state_file" 2>/dev/null) || return 1
    total_steps=$(jq -r '.total_steps' "$state_file" 2>/dev/null) || return 1
    
    if [[ "$status" == "completed" ]]; then
        log_info "Previous run was already completed."
        return 1
    fi
    ...
    log_info "Resuming from step ${current_step}/${total_steps}..."
    return 0
}

# ---------------------------------------------------------------------------
# Graphical Progress Bar (iTerm2/TrueColor/Unicode) - v1.4.0+
# ---------------------------------------------------------------------------

# Render a graphical progress bar with terminal-specific styling
# Usage: progress_graphic <current> <total> [label]
progress_graphic() {
    local current="$1"
    local total="$2"
    local label="${3:-}"
    ...
    # Select rendering style based on terminal capability
    case "$_TERM_CAPABILITY" in
        iterm2|truecolor)
            _render_progress_iterm2 "$current" "$total" "$percentage" "$label"
            ;;
        256color)
            _render_progress_256color "$current" "$total" "$percentage" "$label"
            ;;
        *)
            _render_progress_basic "$current" "$total" "$percentage" "$label"
            ;;
    esac
}

# iTerm2/TrueColor: Use gradient colors and Unicode block characters
_render_progress_iterm2() {
    local current="$1"
    local total="$2"
    local percentage="$3"
    local label="$4"
    ...
    # Build gradient progress bar (green→yellow→red based on progress)
    local bar=""
    ...
    echo -e "\r${bar} ${percentage}% ${label}" >&2
}

# 256color: Use solid colors with Unicode blocks
_render_progress_256color() {
    local current="$1"
    local total="$2"
    local percentage="$3"
    local label="$4"
...
# Basic: ASCII fallback (works everywhere)
_render_progress_basic() {
    local current="$1"
    local total="$2"
    local percentage="$3"
    local label="$4"
...
# Spinner animation (works in any terminal)
...
spinner_start() {
    ...
    (
        ...
        local i=0
        while true; do
            echo -ne "\r${CYAN}${frames[$i]}${NC} ${message}" >&2
            i=$(( (i + 1) % ${#frames[@]} ))
            sleep 0.1
        done
    ) &
...
spinner_stop() {
    if [[ $_spinner_running == true && $_spinner_pid -gt 0 ]]; then
        kill "$_spinner_pid" 2>/dev/null || true
        wait "$_spinner_pid" 2>/dev/null || true
        _spinner_pid=0
        _spinner_running=false
        echo -ne "\r\033[K" >&2  # Clear spinner line
    fi
}
