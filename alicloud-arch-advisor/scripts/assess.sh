...
readonly GLOBAL_TIMEOUT=1800

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "${SCRIPT_DIR}/common.sh"

# Set trap for timeout handling
cleanup_on_timeout() {
    log_error "Operation timed out after ${GLOBAL_TIMEOUT} seconds"
    log_error "Please check network connection or reduce scope"
    exit 124
}
...
# ---------------------------------------------------------------------------
# Main execution (wrapped with timeout)
# ---------------------------------------------------------------------------
main() {
...
timeout "$GLOBAL_TIMEOUT" bash -c 'source "'"$SCRIPT_DIR"'/common.sh"; main'
exit_code=$?

if [[ $exit_code -eq 124 ]]; then
    log_error "操作超时 (${GLOBAL_TIMEOUT}秒)。请检查网络连接或缩小范围。"
    exit 124
