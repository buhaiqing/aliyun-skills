...
| Pillar | Key Guidance |
|--------|-------------|
| **Security** | IAM: `waf:Describe*` for read, `waf:Create*`, `waf:Modify*` for mutating. Use RAM conditions for IP-based access. Enable HTTPS only for production. Combine with DDoS for full protection. Enable SLS integration for audit trail |
| **Stability** | Multi-region deployment. Configure origin health checks. **Scenario:** Monitor traffic anomalies → alert at 80% quota → test failover quarterly |
| **Cost** | Subscription for predictable workloads. **Optimization:** Monitor protected domains count → clean up unused domains → review defense rule quotas |
| **Efficiency** | Use Terraform for repeatable deployments. Full API coverage for automation. Document escalation paths |
| **Performance** | Minimize complex regex in rules. Use caching for static content. Monitor origin response times. Optimize rule evaluation order |

See [references/well-architected-assessment.md](references/well-architected-assessment.md) for full details.
...
|-----------|-------------|
| DescribeInstanceInfo | `InstanceId`, `Edition`, `PayType` |
| DescribeDomainDetail | `DomainId`, `Domain`, `Cname`, `ListenPorts` |
| DescribeIpHitItems | `Ip`, `HitCount` |
| DescribeAccessControlList | `RuleId`, `Status` |
...
> **CRITICAL:** Before executing ANY WAF operation, MUST run the enhanced pre-flight check script to validate environment and detect potential issues early.
...
5. **Region validation** (WAF supports specific regions only)
...
| **CLI Plugin Installation** | waf-openapi plugin, permissions, auto-install | **CRITICAL** |
...
| Region Validation | WAF-supported regions | CRITICAL |
...
2. **Region Validation**:
   - Supported regions: `cn-hangzhou`, `cn-shanghai`, `cn-qingdao`, `cn-beijing`, `cn-zhangjiakou`, `cn-huhehaote`, `cn-shenzhen`, `cn-chengdu`, `cn-hongkong`, `ap-southeast-1`, `ap-southeast-2`, `ap-southeast-3`, `ap-southeast-5`, `us-west-1`, `us-east-1`, `eu-central-1`
   - **Mainland China**: Default to `cn-hangzhou`
...
if [ $PREFLIGHT_STATUS -eq 0 ]; then
    ...
    aliyun waf-openapi DescribeInstanceInfo --RegionId "$ALIBABA_CLOUD_REGION_ID" --version 2021-10-01 --force
...
| Aspect | Setting |
|---|---|
| Required? | **Yes** (Phase 1 rollout) |
| Default `max_iter` | **2** (inherited from `AGENTS.md` §12.8) |
| Operations covered | ALL operations in this SKILL.md |
| Operations most scrutinized | `DeleteDomain`, `DeleteAccessControl`, `DeleteDefenseRule`, `ModifyDomain` (SSL/certificate changes), `CreateAccessControl` (IP blacklist with broad CIDR) |

### Per-Op Safety Sub-Rules (Quick Reference)

For the **full** sub-rule table, see [`references/rubric.md`](../alicloud-waf-ops/references/rubric.md).

| Operation | Hard Safety condition (Score 1 requires) |
|---|---|
| `DeleteDomain` | Explicit user confirmation of `{{user.domain}}`; warn that origin will be directly exposed after removal |
| `DeleteAccessControl` | Explicit user confirmation of `{{user.rule_name}}`; verify rule exists via `DescribeAccessControlList` |
| `DeleteDefenseRule` | Explicit user confirmation of `{{user.rule_name}}`; verify rule exists via `DescribeDefenseRules` |
| `ModifyDomain` | Explicit user confirmation if changing SSL certificates or origin addresses |
| `CreateAccessControl` | **No `0.0.0.0/0` CIDR** unless user explicitly justified; warn that broad CIDR exposes origin to attacks |
| `CreateDefenseRule` | Rate-limiting values validated (RateLimit > 0, RateInterval ≥ 10) |

### WAF-Specific Additions (beyond the 5 core dimensions)

| Dimension | Threshold | Why it matters for WAF |
|---|---|---|
| **Region Compliance** | ≥ 0.5 | `--RegionId` must match `{{user.region_id}}` to avoid cross-region cost leakage |
| **Credential Hygiene** | = 1 (**absolute**) | `ALIBABA_CLOUD_ACCESS_KEY_SECRET` must never appear in any trace field |
| **Well-Architected** | ≥ 0.5 | The 5 WA pillars are scored when the op is WA-sensitive (security / cost) |
...
Every GCL run MUST write `./audit-results/gcl-trace-YYYYMMDD-HHMMSS.json` with the schema defined in `AGENTS.md` §12.6. Apply the WAF-specific sanitization regex helpers to scrub `ALIBABA_CLOUD_ACCESS_KEY_SECRET` before persisting.
