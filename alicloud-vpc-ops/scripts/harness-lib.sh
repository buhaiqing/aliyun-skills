#!/bin/bash
# SkillOpt Core Library for alicloud-vpc-ops
# Self-repair and dynamic optimization for VPC operations.
...
SKILLOPT_LOG_LABEL="VPC-SkillOpt"
SKILLOPT_SKILL_TAG="alicloud-vpc-ops"
...
SKILLOPT_LOG_FILE="${ALIBABA_CLOUD_LOG_DIR:-${_SKILLOPT_LOGS_DIR:-$_SKILLOPT_RUNTIME_ROOT}}/vpc-skillopt-$(date +%Y%m%d).log"
SKILLOPT_RUNTIME_DATA="${_SKILLOPT_METRICS_DATA_DIR:-$_SKILLOPT_RUNTIME_ROOT}/vpc-skillopt-runtime.json"
# --- End shared core ---
...
skillopt_repair_error() {
    ...
    case "$error_code" in
        ...
        ResourceNotFound)
            # VPC resources (vpc-, vsw-, eip- prefixes) — verify existence, retry if found
            skillopt_log "repair[ResourceNotFound]: verify resource via DescribeVpcs"
            local instance_id=""
            local region="${ALIBABA_CLOUD_REGION_ID:-}"
            for ((i=0; i<${#params[@]}; i++)); do
            ...
            if [[ -n "$region" ]]; then
                skillopt_log "repair[ResourceNotFound]: checking region=$region with DescribeVpcs"
                if aliyun vpc DescribeVpcs --RegionId "$region" >/dev/null 2>&1; then
                    skillopt_log "repair[ResourceNotFound]: retrying original command"
        ...
        Forbidden|NoPermission)
            ...
            skillopt_log "HINT: Ensure AK has policy: {\"Action\":[\"vpc:*\"],\"Effect\":\"Allow\",\"Resource\":\"*\"}"
            ;;
