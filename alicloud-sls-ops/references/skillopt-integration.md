# Runtime Harness Integration for alicloud-sls-ops
...
6. **Quota Exceeded** (`SLS`) — notifies user of limits

## Usage

### Direct Command
```bash
aliyun sls DescribeInstances --skillopt-enable --RegionId cn-hangzhou
```

### Wrapper Script
```bash
cd $(pwd)/alicloud-sls-ops
./scripts/sls-skillopt-wrapper.sh DescribeInstances --RegionId cn-hangzhou
```
...
./scripts/sls-harness-wrapper.sh GET / --project my-project

# Check local trace
ls -lt .runtime/traces/alicloud-sls-ops/ | head -3

# Verify in Langfuse UI
# Navigate to https://hai-langfuse-int.hd123.com → Sessions → skill:alicloud-sls-ops
```
