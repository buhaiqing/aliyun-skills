# Runtime Harness Integration for alicloud-actiontrail-ops
...
3. **Resource Not Found** (`ServiceNotExist`) — verifies resource existence
...
### Direct Command
```bash
aliyun actiontrail DescribeInstances --skillopt-enable --RegionId cn-hangzhou
```
...
./scripts/actiontrail-harness-wrapper.sh DescribeTrails

# Check local trace
ls -lt .runtime/traces/alicloud-actiontrail-ops/ | head -3

# Verify in Langfuse UI
# Navigate to https://hai-langfuse-int.hd123.com → Sessions → skill:alicloud-actiontrail-ops
```
