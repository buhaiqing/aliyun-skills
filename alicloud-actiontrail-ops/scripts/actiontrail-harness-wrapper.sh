#!/bin/bash
# ActionTrail Runtime Harness wrapper
set -uo pipefail
...
if [[ ${#} -gt 0 && ("$1" == "actiontrail" || "$1" == "actiontrail2") ]]; then
    PRODUCT="$1"
    shift
fi
...
if [ "$SKILLOPT_LOADED" = true ]; then
    echo "[Wrapper] alicloud-actiontrail-ops/scripts/actiontrail-harness-wrapper.sh ${SUBCMD}" >&2
