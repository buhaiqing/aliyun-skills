#!/bin/bash
# SkillOpt Core Library for alicloud-actiontrail-ops
# Self-repair and dynamic optimization for ActionTrail CLI commands.
...
SKILLOPT_LOG_LABEL="ActionTrail-SkillOpt"
SKILLOPT_SKILL_TAG="alicloud-actiontrail-ops"
...
SKILLOPT_LOG_FILE="${ALIBABA_CLOUD_LOG_DIR:-${_SKILLOPT_LOGS_DIR:-$_SKILLOPT_RUNTIME_ROOT}}/actiontrail-skillopt-$(date +%Y%m%d).log"
SKILLOPT_RUNTIME_DATA="${_SKILLOPT_METRICS_DATA_DIR:-$_SKILLOPT_RUNTIME_ROOT}/actiontrail-skillopt-runtime.json"
# --- End shared core ---
    ...
    case "$error_code" in
        ...
        InvalidParameter|InvalidJSON|MissingParameter|MissingParameter.RegionId|InvalidParameter.RegionId|MissingRegionId|DataRetentionExceeded|InvalidTimeRange|TimeRangeExceeded|InvalidParameter.StartTime|InvalidParameter.TimeRange)
            ...
            if $is_region_error; then
                ...
                if [[ -z "$region" ]]; then
                    if _SKILLOPT_INTERNAL_REPAIR=1 skillopt_run_aliyun actiontrail DescribeRegions --output cols=RegionId rows=Regions.Region 2>/dev/null; then
        ...
        TrailNotFound|ResourceNotFound)
        ...
        Forbidden|NoPermission)
            ...
            skillopt_log "HINT: Ensure AK has policy: {\"Action\":[\"actiontrail:*\"],\"Effect\":\"Allow\",\"Resource\":\"*\"}"
            ;;
        ...
        QuotaExceeded)
            skillopt_log "Quota exceeded for ActionTrail"
            ;;
