#!/bin/bash
# ClickHouse Runtime Harness wrapper
...
PRODUCT="clickhouse"
if [[ ${#} -gt 0 && ("$1" == "clickhouse" || "$1" == "clickhouse2") ]]; then
    PRODUCT="$1"
    shift
fi
...
if [ "$SKILLOPT_LOADED" = true ]; then
    echo "[Wrapper] alicloud-clickhouse-ops/scripts/clickhouse-harness-wrapper.sh ${SUBCMD}" >&2
