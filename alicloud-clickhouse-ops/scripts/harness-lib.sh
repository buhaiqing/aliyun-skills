#!/bin/bash
# SkillOpt Core Library for alicloud-clickhouse-ops
# Self-repair and dynamic optimization for ClickHouse CLI commands.
...
SKILLOPT_LOG_LABEL="ClickHouse-SkillOpt"
SKILLOPT_SKILL_TAG="alicloud-clickhouse-ops"
...
SKILLOPT_LOG_FILE="${ALIBABA_CLOUD_LOG_DIR:-${_SKILLOPT_LOGS_DIR:-$_SKILLOPT_RUNTIME_ROOT}}/clickhouse-skillopt-$(date +%Y%m%d).log"
SKILLOPT_RUNTIME_DATA="${_SKILLOPT_METRICS_DATA_DIR:-$_SKILLOPT_RUNTIME_ROOT}/clickhouse-skillopt-runtime.json"
# --- End shared core ---
...
skillopt_repair_error() {
    ...
    case "$error_code" in
        ...
        ResourceNotFound)
            ...
            local instance_id=""
            for ((i=0; i<${#params[@]}; i++)); do
                if [[ "${params[$i]}" == "--DbInstanceId" && $((i+1)) -lt ${#params[@]} ]]; then
            ...
            if [[ -n "$instance_id" && -n "$region" ]]; then
                ...
                local check_cmd=""
                if [[ "$instance_id" =~ ^cc- ]]; then
                    check_cmd="aliyun clickhouse DescribeDBInstances --RegionId $region --DbInstanceId $instance_id"
                fi
        ...
        Forbidden|NoPermission)
            ...
            skillopt_log "HINT: Ensure AK has policy: {\"Action\":[\"clickhouse:*\"],\"Effect\":\"Allow\",\"Resource\":\"*\"}"
            ;;
        ...
        QuotaExceeded)
            skillopt_log "Quota exceeded for ClickHouse"
            ;;
