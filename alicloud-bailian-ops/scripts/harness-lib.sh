#!/bin/bash
# SkillOpt Core Library for alicloud-bailian-ops
# Self-repair and dynamic optimization for Bailian CLI commands.
...
SKILLOPT_LOG_LABEL="Bailian-SkillOpt"
SKILLOPT_SKILL_TAG="alicloud-bailian-ops"
...
SKILLOPT_LOG_FILE="${ALIBABA_CLOUD_LOG_DIR:-${_SKILLOPT_LOGS_DIR:-$_SKILLOPT_RUNTIME_ROOT}}/bailian-skillopt-$(date +%Y%m%d).log"
SKILLOPT_RUNTIME_DATA="${_SKILLOPT_METRICS_DATA_DIR:-$_SKILLOPT_RUNTIME_ROOT}/bailian-skillopt-runtime.json"
# --- End shared core ---
    ...
    case "$error_code" in
        ...
        InvalidParameter|InvalidJSON|MissingParameter|MissingParameter.RegionId|InvalidParameter.RegionId|MissingRegionId|DataRetentionExceeded|InvalidTimeRange|TimeRangeExceeded|InvalidParameter.StartTime|InvalidParameter.TimeRange)
            ...
            if $is_region_error; then
                ...
                if [[ -z "$region" ]]; then
                    if _SKILLOPT_INTERNAL_REPAIR=1 skillopt_run_aliyun bailian DescribeRegions --output cols=RegionId rows=Regions.Region 2>/dev/null; then
        ...
        Forbidden|NoPermission)
            ...
            skillopt_log "HINT: Ensure AK has policy: {\"Action\":[\"bailian:*\"],\"Effect\":\"Allow\",\"Resource\":\"*\"}"
            ;;
        ...
        QuotaExceeded)
            skillopt_log "Quota exceeded for Bailian"
            ;;
