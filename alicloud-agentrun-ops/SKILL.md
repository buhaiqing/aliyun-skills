...
compatibility: >-
  ...
  endpoints. Also supports AgentRun CLI (`ar`) for terminal/CI scenarios.
metadata:
  author: alicloud
  version: "1.5.0"
  ...
  cli_applicability: "full"
  cli_support_evidence: >-
    AgentRun CLI (`ar` or `agentrun`) provides terminal and CI/CD support for
    Template/Sandbox management and code execution. CLI wraps HTTP APIs with
    pre-configured authentication. See: https://help.aliyun.com/zh/functioncompute/use-the-agentrun-cli-to-manage-agentrun-in-terminals-and-ci-pipelines
  environment:
...
- **`cli_applicability: full`:** AgentRun provides official CLI (`ar` / `agentrun`) for Template/Sandbox management and code execution. CLI wraps HTTP APIs with pre-configured authentication.
- **Dual-path execution:** Choose CLI for terminal/CI scenarios, HTTP API for programmatic/embedded use cases.
- See [AgentRun CLI](#agentrun-cli) section and [references/api-sdk-usage.md](references/api-sdk-usage.md) for details.
...
|---------|------|---------|
| 1.5.0 | 2026-07-08 | Add AgentRun CLI support (`ar` / `agentrun`). Update `cli_applicability: full`. Add CLI installation, configuration, and usage documentation. Create `cli_template_management.sh`, `cli_sandbox_management.sh`, `cli_execute_code.sh` scripts. Add CLI vs HTTP API selection guide. Update api-sdk-usage.md with CLI command mapping. |
| 1.4.0 | 2026-06-04 | Phase 5 `recommended` GCL rollout. Update metadata and quality gates. |
...
> **EXECUTION MANDATORY RULE**: 所有 control-plane CLI 执行步骤 **必须** 通过 SkillOpt wrapper `./scripts/agentrun-harness-wrapper.sh` 运行。
> 以下所有代码块中的 `aliyun agentrun ...` 命令在执行时应替换为 `./scripts/agentrun-harness-wrapper.sh <subcommand> ...`。
> 仅在 wrapper 脚本不可用或 `skillopt-lib.sh` 缺失时，才退回到原生 `aliyun agentrun` CLI 调用。
> 参考 `## Runtime Rules` 中的 CLI path 规则。

## AgentRun CLI

AgentRun CLI (`ar` 或 `agentrun`) 提供命令行方式管理 AgentRun 资源，适用于终端交互、脚本自动化和 CI/CD 流水线场景。
...
# 方式二：环境变量
export AGENTRUN_ACCESS_KEY_ID=$ALIBABA_CLOUD_ACCESS_KEY_ID
...
|------|----------|------|
| 终端交互 | CLI | 预置签名，命令简洁 |
| CI/CD 脚本 | CLI | 退出码规范，易于集成 |
| 嵌入应用程序 | HTTP API | 无需外部依赖 |
| 自定义 HTTP 客户端 | HTTP API | 完整控制请求/响应 |
| 复杂文件操作 | CLI | 内置上传/下载命令 |
...
# 查询 Template
ar template get my-template --output json

# 列出 Templates
ar template list --output table

# 更新 Template
ar template update my-template --cpu 4 --memory 8192

# 删除 Template（带确认）
ar template delete my-template
```text

**脚本化使用**: [assets/code-snippets/cli_template_management.sh](assets/code-snippets/cli_template_management.sh)

```bash
# 使用脚本
chmod +x assets/code-snippets/cli_template_management.sh
...
# 查询 Sandbox
ar sandbox get $SANDBOX_ID --output json

# 列出 Sandboxes
ar sandbox list --status READY --output table

# 停止 Sandbox
ar sandbox stop $SANDBOX_ID --wait

# 暂停 Sandbox (Deep Hibernation)
ar sandbox pause $SANDBOX_ID

# 恢复 Sandbox
ar sandbox resume $SANDBOX_ID --wait

# 删除 Sandbox
ar sandbox delete $SANDBOX_ID --yes
```text

**脚本化使用**: [assets/code-snippets/cli_sandbox_management.sh](assets/code-snippets/cli_sandbox_management.sh)

```bash
# 创建并等待就绪
SANDBOX_ID=$(./assets/code-snippets/cli_sandbox_management.sh create my-template true)

# 暂停以节省成本
./assets/code-snippets/cli_sandbox_management.sh pause $SANDBOX_ID

# 恢复后继续工作
./assets/code-snippets/cli_sandbox_management.sh resume $SANDBOX_ID
...
```bash
# 执行 Python 代码
ar sandbox exec $SANDBOX_ID --language python --code "print(1+1)"

# 执行 JavaScript
ar sandbox exec $SANDBOX_ID --language nodejs --code "console.log('Hello')"

# 执行 Shell 命令
ar sandbox run $SANDBOX_ID --command "ls -la" --cwd /home/user

# 交互式 TTY 终端
ar sandbox tty $SANDBOX_ID
```text

**脚本化使用**: [assets/code-snippets/cli_execute_code.sh](assets/code-snippets/cli_execute_code.sh)

```bash
# 执行代码
./assets/code-snippets/cli_execute_code.sh code $SANDBOX_ID "print('Hello World')" python

# 执行命令
./assets/code-snippets/cli_execute_code.sh run $SANDBOX_ID "uname -a" /home/user
...
| 1 | 资源不存在或失败态 | 检查资源 ID |
| 2 | 参数错误 | 检查命令参数 |
| 3 | 认证/权限失败 | 检查凭证和 RAM 策略 |
| 4 | 服务端错误/超时 | 重试或联系支持 |
| 5 | Runtime 操作失败 | 查看详细日志 |
| 6 | 轮询超时 | 增加 `--timeout` |
| 130 | 用户中断 | 重新执行命令 |

### 混合使用示例

CLI 和 HTTP API 可以混合使用：

```bash
# Step 1: 使用 CLI 快速创建资源
TEMPLATE_NAME="my-cli-template"
ar template create --name $TEMPLATE_NAME --cpu 2 --memory 4096
SANDBOX_ID=$(ar sandbox create --template $TEMPLATE_NAME --output json | jq -r '.sandboxId')

# Step 2: 使用 HTTP API 执行复杂操作（如需自定义请求头）
source assets/code-snippets/sign.py  # 加载签名函数
source assets/code-snippets/execute_code.sh $SANDBOX_ID "import pandas as pd"
...
> **EXECUTION MANDATORY RULE**: 所有 CLI 执行步骤 **必须** 通过 SkillOpt wrapper `./scripts/agentrun-harness-wrapper.sh` 运行。
> 以下所有代码块中的 `aliyun agentrun ...` 命令在执行时应替换为 `./scripts/agentrun-harness-wrapper.sh <subcommand> ...`。
...
- [API & SDK Usage](references/api-sdk-usage.md) — HTTP API 和 CLI 使用指南，命令映射表
- [API Reference](references/api-reference.md) — Full endpoint documentation
- [API Signing Guide](references/api-signing.md) — ACS3-HMAC-SHA256 implementation
