...
### PASS T1: baseline-manager.py 增强 `--compare-with <date>`
...
- PASS 新增 `LocalBackend.get_by_date(date_str)` 方法
- PASS 不存在日期 -> exit=2 + 明确错误 + 列出可用 baselines
- PASS 无效格式 -> exit=2 + Invalid date format 错误

### PASS T2: configdrift.sh 透传 `--compare-with`
...
- PASS 日志开头打印 `COMPARE_WITH: <date> (历史 baseline)` 或 `COMPARE_WITH: latest (默认)`
...
- PASS macOS/Linux `date` 命令差异已加 fallback 注释
...
- PASS `--compare-with 2026/06/07` (无效格式) -> exit=2 + Invalid date format
...
- [x] F8: TODO.md 同步 — PASS（本文件 + 索引 + Hotfix 记录）
