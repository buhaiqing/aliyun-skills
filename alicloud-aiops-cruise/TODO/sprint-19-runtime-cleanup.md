...
- ✅ alicloud-gcl-runner-ops/scripts/ (5 文件): gcl_runner, gcl_actiontrail_crosscheck, gcl_cms_alarm_setup, gcl_passrate_reporter, gcl_runner_test
- ✅ alicloud-aiops-cruise/runbooks/scripts/_shared.py: CACHE_DIR = `audit-results/cache` (280 行)
- ✅ alicloud-aiops-cruise/runbooks/scripts/*.py (7 文件): --output-dir default = `"audit-results"`
- ✅ alicloud-topo-discovery/scripts/lib/baseline_git.py: 无硬编码 (接受参数, 不需改)

### ✅ T2: 迁移 alicloud-gcl-runner-ops 路径

- ✅ gcl_runner.py: trace 输出默认 → `${RUNTIME_ROOT}/audit/gcl-runner/`
- ✅ gcl_actiontrail_crosscheck.py: trace-dir / report 默认 → 同上
- ✅ gcl_cms_alarm_setup.py: --report-dir 默认 → 同上
- ✅ gcl_passrate_reporter.py: trace 扫描 → 同上
- ✅ gcl_runner_test.py: 测试目录 → 用 tmp_path
...
### ✅ T4: 迁移 runbooks 7 个脚本的 --output-dir
...
### ✅ T5: 创建 aliyun-skills-cleanup 命令
...
- ✅ 测试: `test_sprint19_runtime_cleanup.py` (cleanup dry-run + size limit)
- ✅ 测试: `test_sprint19_path_migration.py` (验证所有新默认值正确)
- ✅ 端到端: 跑一次 `gcl_runner` + `daily-health-check` 验证写到 .runtime/
- ✅ F8 同步: SKILL.md Changelog (v1.2.1) + TODO.md 索引
