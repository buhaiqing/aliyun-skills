...
START_TIME_SIX_HOURS=$(date -u -d '6 hours ago' +%Y-%m-%dT%H:%M:%SZ 2>/dev/null || date -u -v-6H +%Y-%m-%dT%H:%M:%SZ 2>/dev/null)  # 6小时前

# 昨日同期
YESTERDAY_START=$(date -u -d '30 hours ago' +%Y-%m-%dT%H:%M:%SZ 2>/dev/null || date -u -v-30H +%Y-%m-%dT%H:%M:%SZ 2>/dev/null)
YESTERDAY_END=$(date -u -d '24 hours ago' +%Y-%m-%dT%H:%M:%SZ 2>/dev/null || date -u -v-24H +%Y-%m-%dT%H:%M:%SZ 2>/dev/null)
```
...
aliyun cms DescribeMetricList \
  ...
  --Period 3600 \
  --StartTime "$(date -u -d '7 days ago' +%Y-%m-%dT%H:%M:%SZ 2>/dev/null || date -u -v-7d +%Y-%m-%dT%H:%M:%SZ 2>/dev/null)" --EndTime "$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
  | jq '.Datapoints | fromjson | [.[].Maximum] | max // -1'
