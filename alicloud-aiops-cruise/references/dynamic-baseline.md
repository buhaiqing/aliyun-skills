...
动态基线:  |current - μ| / σ > 2.0 = Warning
  ├─ 白天 CPU 65%（μ=55%, σ=5%) → Z=2.0→Warning ✅ 凌晨 CPU 30%（μ=10%, σ=3%) → Z=6.7→Critical ✅
  └─ 自动适应不同业务的"正常值"
```
...
if [ "$(echo "$CURRENT > $P99" | bc -l)" = "1" ]; then echo "🔴 CRITICAL"
elif [ "$(echo "$CURRENT > $P95" | bc -l)" = "1" ]; then echo "🟡 WARNING"
else echo "✅ OK"; fi
```
...
|---|---|---|---|---|---|
| 🔴 Critical | Z > 3.0 | > P99 | 🔴 | 列入 Critical 清单 | 立即介入排查 |
| 🟡 Warning | Z > 2.0 | > P95 | 🟡 | 列入 Warning 清单 | 关注趋势 |
| 🔵 Info | Z > 1.0 | > P75 | 🔵 | 仅在异常评分摘要中展示 | 记录，不告警 |
| ✅ Normal | Z ≤ 1.0 | ≤ P75 | ✅ | 不展示 | 无操作 |
...
例 1: CPU 当前 25%, 固定阈值 → ✅ OK, Z-Score = 3.5 → 🔴 Critical
  最终 → 🔴 Critical（基线发现异常，阈值漏报）

例 2: CPU 当前 75%, 固定阈值 → 🟡 Warning, Z-Score = 1.5 → ✅ OK
