...
def q(cmd: list, timeout=30) -> dict | None:
    ...
    for attempt in range(_MAX_RETRIES if is_cms else 1):
        # 非阻塞获取信号量,避免死锁
        if is_cms and not _CMS_SEM.acquire(timeout=timeout):
            warn("E429", f"semaphore timeout for {' '.join(cmd)}")
            return None
        try:
...
def gate(region: str) -> bool:
    ...
    if not os.environ.get("ALIBABA_CLOUD_ACCESS_KEY_SECRET"):
        ...
        return False
    for t in ["aliyun"]:
        if not subprocess.run(["which", t], capture_output=True, timeout=5).returncode == 0:
...
def to_incident(finding: dict, *, customer: str, run_id: str, region: str, runbook_id: str, runbook_version: str, scenario: str, report_path: str, level_override: str | None = None) -> dict:
    ...
    # ✅ F-004 修复: timestamp 使用标准 ISO8601 格式
    timestamp = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")

    # ✅ F-004 修复: dedup_key 校验
    dedup_key = _gen_dedup_key(customer, resource_type, resource_id, rule_id, level)
    if level != "INFO" and not dedup_key:
        # CRITICAL/WARNING 必须有 dedup_key,否则抛出异常
        raise ValueError(f"CRITICAL/WARNING incident missing dedup_key: customer={customer}, resource_type={resource_type}, resource_id={resource_id}, rule_id={rule_id}")

    return {
        "incident_id": str(uuid.uuid4()),
        "schema_version": "1.0.0",
        "customer": customer,
        "timestamp": timestamp,  # ✅ 修复后
        "run_id": run_id,
        "level": level,
        ...
        "rule_version": "1.0.0",
        "title": title,
        "dedup_key": dedup_key or "",  # INFO 级别允许空字符串
        "metric": metric,
        "current_value": current_value,
