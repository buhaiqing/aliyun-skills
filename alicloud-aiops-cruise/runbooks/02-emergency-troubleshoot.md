...
WINDOW_START=$(date -u -j -f "%Y-%m-%dT%H:%M:%SZ" "$ALERT_TIME" "+%Y-%m-%dT%H:%M:%SZ" 2>/dev/null || echo "$(date -u -d '1 hour ago' +%Y-%m-%dT%H:%M:%SZ 2>/dev/null || date -u -v-1H +%Y-%m-%dT%H:%M:%SZ 2>/dev/null)")
...
for INST_ID in $ECS_IDS; do
  ...
  CPU_PEAK=$(aliyun cms DescribeMetricList \
    ...
    --Period 60 \
    --StartTime "$(date -u -d '30 minutes ago' +%Y-%m-%dT%H:%M:%SZ 2>/dev/null || date -u -v-30M +%Y-%m-%dT%H:%M:%SZ 2>/dev/null)" \
    --EndTime "$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
