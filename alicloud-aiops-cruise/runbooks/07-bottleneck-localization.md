...
if [ -n "$ALERT_TIME" ]; then
  WINDOW_START=$(date -u -j -f "%Y-%m-%dT%H:%M:%SZ" "$ALERT_TIME" -v-30M +%Y-%m-%dT%H:%M:%SZ 2>/dev/null)
  WINDOW_END=$(date -u -j -f "%Y-%m-%dT%H:%M:%SZ" "$ALERT_TIME" -v+30M +%Y-%m-%dT%H:%M:%SZ 2>/dev/null)
else
