...
if [ -n "$RESOURCE_ID" ] && [ -z "$RG_ID" ]; then
  case "${RESOURCE_ID:0:2}" in
    i-)  RG_ID=$(aliyun ecs DescribeInstances --RegionId "$REGION" \
           --InstanceIds "[\"$RESOURCE_ID\"]" | jq -r '(.Instances.Instance[0].ResourceGroupId | if (. | type) == "array" then (if length > 0 then .[0] else "default" end) else . end) // "default" | if . == "" then "default" else . end') ;;
    rm-|pgm-) RG_ID=$(aliyun rds DescribeDBInstances --RegionId "$REGION" \
           --DBInstanceId "$RESOURCE_ID" | jq -r '.Items.DBInstance[0].ResourceGroupId // "default" | if . == "" then "default" else . end') ;;
    r-)  RG_ID=$(aliyun r-kvstore DescribeInstances --RegionId "$REGION" | jq -r \
           --arg id "$RESOURCE_ID" '(.Instances.KVStoreInstance[] | select(.InstanceId==$id) | .ResourceGroupId | if (. | type) == "array" then (if length > 0 then .[0] else "default" end) else . end) // "default"') ;;
    lb-) RG_ID=$(aliyun slb DescribeLoadBalancers --RegionId "$REGION" | jq -r \
           --arg id "$RESOURCE_ID" '(.LoadBalancers.LoadBalancer[] | select(.LoadBalancerId==$id) | .ResourceGroupId | if (. | type) == "array" then (if length > 0 then .[0] else "default" end) else . end) // "default"') ;;
    ngw-) RG_ID=$(aliyun vpc DescribeNatGateways --RegionId "$REGION" \
           --NatGatewayId "$RESOURCE_ID" | jq -r '.NatGateways.NatGateway[0].ResourceGroupId // "default" | if . == "" then "default" else . end') ;;
    eip-) RG_ID=$(aliyun vpc DescribeEipAddresses --RegionId "$REGION" \
           --AllocationId "$RESOURCE_ID" | jq -r '.EipAddresses.EipAddress[0].ResourceGroupId // "default" | if . == "" then "default" else . end') ;;
    vpc-) RG_ID=$(aliyun vpc DescribeVpcs --RegionId "$REGION" --VpcId "$RESOURCE_ID" \
           | jq -r '.Vpcs.Vpc[0].ResourceGroupId // "default" | if . == "" then "default" else . end') ;;
  esac
...
echo "  扫描策略: 资源组优先 + 标签补充"
  # ── 资源组合法性校验 ──
  # B-2 Blocker: 阿里云 ECS API 将"默认资源组"表示为空字符串 ""，而非 "default"
  # 这导致 --ResourceGroupId "rg-xxx" 扫描会 100% 漏掉在默认 RG 的 ECS（API 层面就匹配不到空字符串）
  # jq 筛选对 RG_NO 产品（SG/VPC/EIP）可以正确匹配空字符串，但 ECS/ACK 的 --ResourceGroupId 参数无法匹配
  # 故预检仅跳过用户显式输入 "default" 的情况，不清空用户提供的有效 RG ID
  if [ "$RG_ID" = "default" ] || [ "$RG_ID" = "rg-default" ]; then
    echo "  [WARN] 显式 default 标识符，跳过 RG 过滤，改为全量扫描"
    RG_ID=""
  elif [ -n "$RG_ID" ] && [ "$RG_ID" != "" ]; then
    echo "  [PKG] 资源组: $RG_ID PASS (自定义资源组)"
    echo "  [INFO] 注意: ECS/ACK 在默认 RG 时，--ResourceGroupId 过滤会返回 0 台"
    echo "  [INFO]   原因: API 用空字符串 "" 表示默认 RG，非 "default""
  else
    echo "  [PKG] 资源组: (未提供，扫描全部)"
  fi
echo "═══════════════════════════════════════"
...
REDIS_COUNT=$(echo "${REDIS_LIST_RG:-null}" | jq 'if . == null then 0 else length end')
...
for LB_ID in $(echo "$SLB_LIST" | jq -r '.LoadBalancerId'); do
  # DescribeVServerGroups 仅返回虚拟服务器组，使用默认服务器组的 SLB 返回空数组
  VS_COUNT=$(aliyun slb DescribeVServerGroups \
    --RegionId "$REGION" \
    --LoadBalancerId "$LB_ID" | jq '.VServerGroups.VServerGroup | if (type == "array") then length else 0 end')
  if [ "$VS_COUNT" = "0" ]; then
    echo "[DIAG] SLB $LB_ID: 默认服务器组（无虚拟服务器组）"
  else
    BACKEND_SERVERS=$(aliyun slb DescribeVServerGroups \
      --RegionId "$REGION" \
      --LoadBalancerId "$LB_ID" | jq '[.VServerGroups.VServerGroup[].VServerGroupId]')
    echo "[DIAG] SLB $LB_ID 虚拟服务器组: $VS_COUNT 个 IDs=$BACKEND_SERVERS"
  fi
done
...
DAILY_BASELINE_START=$(date -u -d '30 hours ago' +%Y-%m-%dT%H:%M:%SZ 2>/dev/null || date -u -v-30H +%Y-%m-%dT%H:%M:%SZ 2>/dev/null)
DAILY_BASELINE_END=$(date -u -d '24 hours ago' +%Y-%m-%dT%H:%M:%SZ 2>/dev/null || date -u -v-24H +%Y-%m-%dT%H:%M:%SZ 2>/dev/null)
...
for NAT_ID in $(aliyun vpc DescribeNatGateways --RegionId "$REGION" | jq -r '.NatGateways.NatGateway[].NatGatewayId'); do
  ...
  NAT_SNAT=$(aliyun cms DescribeMetricList \
    ...
    --Period 3600 \
    --StartTime "$(date -u -d '7 days ago' +%Y-%m-%dT%H:%M:%SZ 2>/dev/null || date -u -v-7d +%Y-%m-%dT%H:%M:%SZ 2>/dev/null)" --EndTime "$END_TIME" \
    | jq '.Datapoints | fromjson | [.[].Maximum] | max // -1')
...
> **触发条件**：当 Step 2.1 中 ECS 的 `DiskReadIOPS` 或 `DiskWriteIOPS` > 70% 云盘上限，或 `LoadAverage` > CPU×2 时，自动触发本步骤。
> **诊断方法**：通过 Cloud Assistant 执行 `iostat` + `iotop` + `free` + `mount` 脚本，详见 `alicloud-ecs-ops/references/host-io-inspection.md`。

```bash
IO_INSTANCES=""
for INST_ID in $(echo "$ECS_LIST" | jq -r '.[].InstanceId'); do
  # 从 Step 2.1 已采集的指标中筛选 IO 异常实例
  INST_CPU=$(echo "$ECS_LIST" | jq -r --arg id "$INST_ID" '.[] | select(.InstanceId==$id) | .Cpu')
  INST_IOPS=$(echo "$ECS_LIST" | jq -r --arg id "$INST_ID" '.[] | select(.InstanceId==$id) | .DiskReadIOPS // 0')
  
  # 判定：IOPS > 70% 云盘上限 或 LoadAverage > CPU×2
  if [ "$INST_IOPS" != "" ] && [ "$INST_IOPS" != "0" ] && [ "$(echo "$INST_IOPS > 3500" | bc -l 2>/dev/null)" = "1" ]; then
    IO_INSTANCES="$IO_INSTANCES $INST_ID"
  fi
done

# 对 IO 异常实例执行 Host 诊断
for INST_ID in $IO_INSTANCES; do
  IO_SCRIPT='#!/bin/bash
echo "=== IOSTAT (3x2s) ==="
...
'
  ...
  IO_RESULT=$(aliyun ecs DescribeInvocationResults \
    --RegionId "$REGION" \
    --InstanceId "$INST_ID" \
    --CommandId "$CMD_ID" | jq -r '.Invocation.InvocationResults.InvocationResult[0].Output // ""' | base64 -d 2>/dev/null)
  
  # 解析关键指标
  IO_UTIL=$(echo "$IO_RESULT" | grep -A5 "iostat" | awk '/^[sv]d[a-z]/{print $(NF-2)}' | head -1)
  IO_AWAIT=$(echo "$IO_RESULT" | grep -A5 "iostat" | awk '/^[sv]d[a-z]/{print $(NF-1)}' | head -1)
  SWAP_USED=$(echo "$IO_RESULT" | grep -A2 "SwapTotal" | awk '{print $2-$3}')
  DIRTY=$(echo "$IO_RESULT" | grep "Dirty:" | awk '{print $2}')
  
  # 异常判定
  IO_STATUS="PASS"
  if [ "$(echo "$IO_UTIL > 80" | bc -l 2>/dev/null)" = "1" ]; then
    IO_STATUS="CRITICAL"
    echo "[CRITICAL] ECS $INST_ID IO %util=${IO_UTIL}% > 80% -> IOPS/吞吐达上限"
  elif [ "$(echo "$IO_UTIL > 60" | bc -l 2>/dev/null)" = "1" ]; then
    IO_STATUS="WARNING"
    echo "[WARNING] ECS $INST_ID IO %util=${IO_UTIL}% > 60%"
  fi
  
  if [ "$(echo "$IO_AWAIT > 50" | bc -l 2>/dev/null)" = "1" ]; then
    IO_STATUS="CRITICAL"
    echo "[CRITICAL] ECS $INST_ID IO await=${IO_AWAIT}ms > 50ms -> IO 延迟严重"
  elif [ "$(echo "$IO_AWAIT > 10" | bc -l 2>/dev/null)" = "1" ] && [ "$IO_STATUS" = "PASS" ]; then
    IO_STATUS="WARNING"
    echo "[WARNING] ECS $INST_ID IO await=${IO_AWAIT}ms > 10ms"
  fi
  
  if [ "$SWAP_USED" != "" ] && [ "$(echo "$SWAP_USED > 102400" | bc -l 2>/dev/null)" = "1" ]; then
    IO_STATUS="WARNING"
    echo "[WARNING] ECS $INST_ID SwapUsed=${SWAP_USED}KB -> 内存不足导致 swap IO"
  fi
...
```bash
# 基线窗口: 最近 7 天 (1h 粒度)
BASELINE_START=$(date -u -d '7 days ago' +%Y-%m-%dT%H:%M:%SZ 2>/dev/null || date -u -v-7d +%Y-%m-%dT%H:%M:%SZ 2>/dev/null)
