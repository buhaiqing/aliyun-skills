...
echo "  扫描策略: 资源组优先 + 标签补充"
  ...
  if [ -n "$RG_ID" ] && [ "$RG_ID" != "" ]; then
    if echo "$RG_ID" | grep -qiE "^(default|rg-default)"; then
      echo "  ⚠️ 默认资源组($RG_ID)跳过，需用户明确要求全账号扫描"
      RG_ID=""
    else
      echo "  📦 资源组: $RG_ID ✅ (自定义资源组)"
    fi
  else
    echo "  📦 资源组: (未提供或为空，跳过)"
  fi
echo "═══════════════════════════════════════"
[ -n "$RG_ID" ] && echo "  📦 资源组: $RG_ID" || echo "  📦 资源组: (未提供)"
[ -n "$TAG_KEY" ] && echo "  🏷️ 标签:   $TAG_KEY=$TAG_VALUE" || echo "  🏷️ 标签:   (未提供)"
```
...
> ⚠️ 注意：EIP 的 CloudMonitor 指标命名空间为 `acs_vpc_eip`，指标名为 `net_in.rate_percentage`（入方向带宽使用率）、`net_out.rate_percentage`（出方向带宽使用率）。
...
> ⚠️ 注意：NAT 的 `SnatConnection` 指标在低流量环境可能无数据（返回空数组），此时查 `EniSessionActiveConnection`（活跃连接数）作为替代。端口分配失败 `EniPacketsDropPortAllocationFail` > 0 说明出现过 SNAT 端口耗尽 —— 这是阿里云 NAT 网关最常见的故障模式。
...
evaluate() {
  ...
  if [ "$(echo "$value >= $crit" | bc -l 2>/dev/null)" = "1" ]; then echo "🔴 CRITICAL"
  elif [ "$(echo "$value >= $warn" | bc -l 2>/dev/null)" = "1" ]; then echo "🟡 WARNING"
  else echo "✅ OK"; fi
}
```
...
| 资源 | 指标 | 趋势 | 预计达阈值日 | 建议 |
|---|---|---|---|---|
| i-xxx | 磁盘使用率 | ↑ 线性增长 | 2026-06-20 | 扩容磁盘或清理 |
| rm-xxx | 磁盘使用率 | ↑ 线性增长 | 2026-07-01 | DAS查空间分析 |
| r-xxx | 内存使用率 | → 平稳 | 无风险 | — |
```
