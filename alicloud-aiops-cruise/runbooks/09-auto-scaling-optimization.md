...
D7_START=$(date -u -d '7 days ago' +%Y-%m-%dT%H:%M:%SZ 2>/dev/null || date -u -v-7d +%Y-%m-%dT%H:%M:%SZ 2>/dev/null)
D30_START=$(date -u -d '30 days ago' +%Y-%m-%dT%H:%MZ 2>/dev/null || date -u -v-30d +%Y-%m-%dT%H:%MZ 2>/dev/null)
H6_START=$(date -u -d '6 hours ago' +%Y-%m-%dT%H:%M:%SZ 2>/dev/null || date -u -v-6H +%Y-%m-%dT%H:%M:%SZ 2>/dev/null)
...
for SG_ID in $(echo "$ESS_LIST" | jq -r '.[].ScalingGroupId // empty'); do
  ...
  SCALING_ACTIVITIES=$(aliyun ess DescribeScalingActivities \
    ...
    | jq --arg h24 "$(date -u -d '24 hours ago' +%Y-%m-%dT%H:%M:%SZ 2>/dev/null || date -u -v-24H +%Y-%m-%dT%H:%M:%SZ 2>/dev/null)" \
