#!/usr/bin/env bash
# Claude Code SessionStart — shared HARNESS_SESSION_ID + W3C traceparent root.
set -euo pipefail
...
export HARNESS_CODING_AGENT="${HARNESS_CODING_AGENT:-claude_code}"

sid="$(agent_turn_resolve_session_id claude_code "$input" 2>/dev/null || true)"
if [[ -n "$sid" ]]; then
