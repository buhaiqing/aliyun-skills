---
name: alicloud-link-cruise
version: "1.0.0"
metadata:
  description: >-
    阿里云全链路巡检 Skill — 从 EIP→SLB→ECS→RDS/Redis→NAT→安全组的端到端健康巡检、
...
| **和 topo-discovery 有什么区别？** | `topo-discovery` 做静态拓扑发现和 HCL 导出；`link-cruise` 做动态健康巡检（含监控、诊断、推理）。拓扑发现是链路巡检的"前置步骤"而非终点 |
| **和 cms-ops 有什么区别？** | `cms-ops` 查单个产品的监控指标；`link-cruise` 跨产品组合指标做链路关联推理（例如：SLB 健康检查失败 + ECS 正常 = 查网络连通性） |
