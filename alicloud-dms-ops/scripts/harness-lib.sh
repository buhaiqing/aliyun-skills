#!/bin/bash
# SkillOpt Core Library for alicloud-dms-ops
# Self-repair and dynamic optimization for DMS CLI commands.
...
SKILLOPT_LOG_LABEL="DMS-SkillOpt"
SKILLOPT_SKILL_TAG="alicloud-dms-ops"
...
SKILLOPT_LOG_FILE="${ALIBABA_CLOUD_LOG_DIR:-${_SKILLOPT_LOGS_DIR:-$_SKILLOPT_RUNTIME_ROOT}}/dms-skillopt-$(date +%Y%m%d).log"
SKILLOPT_RUNTIME_DATA="${_SKILLOPT_METRICS_DATA_DIR:-$_SKILLOPT_RUNTIME_ROOT}/dms-skillopt-runtime.json"
# --- End shared core ---
...
skillopt_repair_error() {
    ...
    case "$error_code" in
        ...
        InvalidParameter|InvalidJSON|MissingParameter|MissingParameter.RegionId|InvalidParameter.RegionId|MissingRegionId|DataRetentionExceeded|InvalidTimeRange|TimeRangeExceeded|InvalidParameter.StartTime|InvalidParameter.TimeRange|SqlSyntaxError|DbTypeNotSupported)
            ...
            if $is_region_error; then
                ...
                if command -v jq >/dev/null 2>&1; then
                    ...
                    skillopt_log "repair[InvalidParam]: jq not found, skipping JSON validation"
                fi
                for ((i=0; i<${#params[@]}; i++)); do
                    local arg="${params[$i]}"
                    if $skip_next; then
        ...
        ResourceNotFound)
            ...
            # DMS ResourceNotFound refers to database/task/user/table not found.
            # Probe by checking whether the referenced DbId still exists via ListDatabases.
            local db_id=""
            local has_dbid_param=false
            for ((i=0; i<${#params[@]}; i++)); do
                if [[ "${params[$i]}" == "--DbId" && $((i+1)) -lt ${#params[@]} ]]; then
                    db_id="${params[$((i+1))]}"
                    has_dbid_param=true
                    break
                fi
            ...
            if $has_dbid_param; then
                skillopt_log "repair[ResourceNotFound]: checking DbId $db_id"
                local exists=false
                if aliyun dms ListDatabases --PageSize 100 2>/dev/null | grep -q "\"DbId\":\"$db_id\""; then
                    exists=true
                fi
                if $exists; then
                    skillopt_log "repair[ResourceNotFound]: DbId $db_id exists, retrying"
                    if skillopt_run_aliyun "$product" "$action" "${params[@]}"; then
                        repair_failed=0
                    fi
                else
                    skillopt_log "repair[ResourceNotFound]: DbId $db_id not found"
                fi
            else
                # Cannot identify the resource, do not retry blindly
                skillopt_log "repair[ResourceNotFound]: cannot extract DbId, skipping"
            fi
            ;;

        Forbidden|NoPermission|Unauthorized)
            skillopt_log "repair[Forbidden]: RAM policy suggestion"
            skillopt_log "HINT: Ensure AK has policy: {\"Action\":[\"dms:*\"],\"Effect\":\"Allow\",\"Resource\":\"*\"}"
            ;;
        ...
        InternalError)
            skillopt_log "repair[InternalError]: server error, retry 3x (2s,4s,8s backoff)"
            local attempt=0
            for sleep_s in 2 4 8; do
        ...
        QuotaExceeded)
            skillopt_log "Quota exceeded for DMS"
            ;;

        HighRiskSQL.Blocked|PermissionDenied|AccessDeniedForUser|ApproveNotAllowed|RejectNotAllowed|OrderNotAllow|OwnerNotAllowed|UserAlreadyExists|DatabaseAlreadyRegistered|LockNotFound|LockConflict|ConnectionFailed|SensitiveColumnAccessDenied|ProcOrFuncNotFound|OutParamNotReturned)
            # DMS business errors that require human intervention, not auto-retry.
            skillopt_log "repair[$error_code]: HALT — human intervention required (see troubleshooting.md)"
            ;;
