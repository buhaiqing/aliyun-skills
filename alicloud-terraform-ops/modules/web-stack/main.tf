...
module "polardb" {
  count  = var.enable_polardb ? 1 : 0
  source = "../addon-polardb"
  ...
  polardb_instance_class = var.polardb_instance_class
}
...
module "alb" {
  ...
  vpc_id         = module.network.vpc_id
  vswitch_id     = module.network.primary_vswitch_id
  zone_id        = module.network.primary_zone_id
}
