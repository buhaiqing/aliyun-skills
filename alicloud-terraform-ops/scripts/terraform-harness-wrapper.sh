#!/bin/bash
# Terraform Runtime Harness wrapper
# Graceful fallback: sources harness-lib.sh (or legacy skillopt-lib.sh symlink) if available.
set -uo pipefail
...
PRODUCT="terraform"
if [[ ${#} -gt 0 && ("$1" == "terraform" || "$1" == "terraform2") ]]; then
    PRODUCT="$1"
    shift
fi
...
if [ "$SKILLOPT_LOADED" = true ]; then
    echo "[Wrapper] alicloud-terraform-ops/scripts/terraform-harness-wrapper.sh ${SUBCMD}" >&2
