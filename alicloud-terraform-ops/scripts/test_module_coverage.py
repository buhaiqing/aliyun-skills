...
from nl2hcl_generator import NL2HCLGenerator, CoverageGapError
...
class ModuleCoverageGateTest(unittest.TestCase):
    ...
    def test_mongodb_keyword_halts(self):
        gen = NL2HCLGenerator()
        request = "创建 VPC 和 MongoDB 集群"
        intent = gen.parse_intent(request)
        report = check_nl2hcl_coverage(intent, request)
        self.assertTrue(report.must_halt)
        self.assertTrue(any(g.tf_type == "alicloud_mongodb_instance" for g in report.gaps))

    def test_oss_keyword_halts(self):
        request = "创建 OSS 存储桶"
