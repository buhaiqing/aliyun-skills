...
class NL2HCLGenerator:
    ...
    RESOURCE_PATTERNS = {
        ...
        r"slb|负载均衡|clb": "alicloud_slb",
        r"nat|网关": "alicloud_nat_gateway",
        r"eip|弹性ip|弹性公网ip": "alicloud_eip",
        ...
        r"mongodb|mongo|文档数据库|dds": "alicloud_mongodb_instance",
        r"oss|对象存储|存储桶|bucket": "alicloud_oss_bucket",
    ...
    ENV_DEFAULTS = {
        "int": {
            ...
            "mongodb_class": "dds.mongo.shard.small",
            "polardb_class": "polar.mysql.x4.medium",
        },
        "dev": {
        ...
        "staging": {
            ...
            "mongodb_class": "dds.mongo.standard.xlarge",
            "polardb_class": "polar.mysql.x4.large",
        },
        "production": {
            "vpc_cidr": "10.2.0.0/16",
            "instance_type": "ecs.g7.2xlarge",
            "rds_class": "rds.mysql.x4.large",
            "mongodb_class": "dds.mongo.standard.4xlarge",
            "polardb_class": "polar.mysql.x8.xlarge",
        },
    }
    ...
    def generate(self, request: str, output_dir: Path | None = None) -> dict[str, str]:
