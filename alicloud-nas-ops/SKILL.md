...
> **EXECUTION MANDATORY RULE**: 所有 control-plane CLI 执行步骤 **必须** 通过 SkillOpt wrapper `./scripts/nas-skillopt-wrapper.sh` 运行。
...
- [GCL Rubric](references/rubric.md) — Phase 5 extension GCL rubric (file system delete, mount target, recycle bin)
...
| Most-scrutinized | `DeleteFileSystem` (mount-target cascade + snapshot backup), `DisableAndCleanRecycleBin` (permanent purge) |
| Cross-skill delegation | `alicloud-vpc-ops` for mount-target VPC/vSwitch validation |
