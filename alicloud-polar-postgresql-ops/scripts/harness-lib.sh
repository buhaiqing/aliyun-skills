#!/bin/bash
# SkillOpt Core Library for alicloud-polar-postgresql-ops
# Self-repair and dynamic optimization for PolarDB PostgreSQL CLI commands.
...
SKILLOPT_LOG_LABEL="PolarDB-PostgreSQL-SkillOpt"
SKILLOPT_SKILL_TAG="alicloud-polar-postgresql-ops"
...
SKILLOPT_LOG_FILE="${ALIBABA_CLOUD_LOG_DIR:-${_SKILLOPT_LOGS_DIR:-$_SKILLOPT_RUNTIME_ROOT}}/polar-postgresql-skillopt-$(date +%Y%m%d).log"
SKILLOPT_RUNTIME_DATA="${_SKILLOPT_METRICS_DATA_DIR:-$_SKILLOPT_RUNTIME_ROOT}/polar-postgresql-skillopt-runtime.json"
# --- End shared core ---
...
skillopt_repair_error() {
    ...
    case "$error_code" in
        ...
        ResourceNotFound)
            ...
            if [[ -n "$instance_id" && -n "$region" ]]; then
                ...
                if [[ "$instance_id" =~ ^i- ]]; then
                    ...
                    check_cmd="aliyun r-kvstore DescribeInstances --RegionId '$region' --InstanceId '$instance_id'"
                    ...
                    check_cmd="aliyun cs DescribeClusterDetail --ClusterId '$instance_id'"
                elif [[ "$instance_id" =~ ^(nas-|file-) ]]; then
                    check_cmd="aliyun nas DescribeFileSystems --FileSystemId '$instance_id'"
                fi
        ...
        Forbidden|NoPermission)
            ...
            skillopt_log "HINT: Ensure AK has policy: {\"Action\":[\"polardb:*\"],\"Effect\":\"Allow\",\"Resource\":\"*\"}"
            ;;
