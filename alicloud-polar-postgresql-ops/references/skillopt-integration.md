# Runtime Harness Integration for alicloud-polar-postgresql-ops
...
### Wrapper Script
```bash
cd $(pwd)/alicloud-polar-postgresql-ops
./scripts/polar-postgresql-skillopt-wrapper.sh DescribeDBClusters --RegionId cn-hangzhou
```
...
./scripts/polar-postgresql-harness-wrapper.sh DescribeDBClusters --RegionId cn-hangzhou

# Check local trace
ls -lt .runtime/traces/alicloud-polar-postgresql-ops/ | head -3

# Verify in Langfuse UI
# Navigate to https://hai-langfuse-int.hd123.com → Sessions → skill:alicloud-polar-postgresql-ops
```
