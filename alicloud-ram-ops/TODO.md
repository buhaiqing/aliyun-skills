...
## 2026-06-16 — v2.4.0 AIOps Content Layering + Monitoring

**Why:** Per AGENTS.md §10.1 TE-7, AIOps/FinOps content should be lazy-loaded
in `references/advanced/`. RAM had an `intelligent-inspection.md` in the
always-loaded layer. Also missing `references/monitoring.md` which is a
canonical requirement (AGENTS.md §1).

**Change scope:** 3 new files + 1 redirect + SKILL.md update.

| # | Status | Item | Notes |
|---|--------|------|-------|
| 1 | ✅ | Create `references/monitoring.md` (CMS metrics, ActionTrail events, security monitoring, anomaly patterns, alert storm handling) | 170 lines; new file |
| 2 | ✅ | Create `references/advanced/aiops-ram.md` (anomaly detection, predictive analysis, auto-remediation, security posture scoring, cross-skill orchestration) | 210 lines; new file |
| 3 | ✅ | Create `references/advanced/intelligent-inspection.md` (full security audit runbook, scoring, remediation, scheduled inspection) | 200 lines; upgraded from original |
| 4 | ✅ | Replace `references/intelligent-inspection.md` with redirect to `references/advanced/intelligent-inspection.md` | Redirect preserves backward compatibility |
| 5 | ✅ | Add "安全巡检" row to `references/prompt-examples.md` Chinese interaction table | Cross-reference |
| 6 | ✅ | Update SKILL.md Reference Directory: add monitoring.md + advanced/aiops-ram.md, update intelligent-inspection.md path | 3 new entries |
| 7 | ✅ | Bump frontmatter `version` to `2.4.0` and `last_updated` to `2026-06-16` | Following metadata convention |
...
| B2 | [x] | Add `assets/eval_queries.json` (trigger-accuracy eval set) | P1 | ✅ Already exists (5 queries) |
| B3 | [x] | Add `assets/example-config.yaml` (RAM env / role / policy examples) | P2 | ✅ Already exists |
| B4 | [x] | Add `references/monitoring.md` (CMS metrics for RAM) | P2 | ✅ Created in v2.4.0 |
| B5 | [x] | Generate advanced/ subfolder (aiops- RAM topics) | P3 | ✅ Created `advanced/aiops-ram.md` + `advanced/intelligent-inspection.md` in v2.4.0 |
| B6 | [x] | Promote `references/intelligent-inspection.md` (智能巡检) to be a full advanced/aiops-* doc | P3 | ✅ Promoted to `references/advanced/intelligent-inspection.md` in v2.4.0 |
| B7 | [x] | Reconcile `prompt-examples.md` (intent → action) with `references/intelligent-inspection.md` (security巡检) | P3 | ✅ Cross-references added in v2.4.0 |

## 2026-06-17 — v2.4.1 Langfuse Integration Fixes

**Why:** Fix Langfuse integration per task requirements

**Change scope:** Delete duplicate skillopt-lib.sh from references/, verify wrapper source path, confirm TODO.md and SKILL.md compliance.

| # | Status | Item | Notes |
|---|--------|------|-------|
| 1 | ✅ | Delete duplicate skillopt-lib.sh from references/ directory | Removed references/skillopt-lib.sh |
| 2 | ✅ | Ensure wrapper script sources scripts/skillopt-lib.sh correctly | Already correct: sources "$SCRIPT_DIR/skillopt-lib.sh" |
| 3 | ✅ | Update TODO.md with post-update self-review checklist | Added this section |
| 4 | ✅ | Verify SKILL.md has mandatory Runtime Rules for SkillOpt wrapper | Already present |
...
- **F6 (token efficiency):** the refactor is precisely the TE-6 (eliminate cross-file duplication) and TE-7 (layer professional content) optimization mandated by AGENTS.md §10.1.
...
- **F8 (TODO.md sync):** this file exists and is updated as part of the v2.4.0 and v2.4.1 refactors.
