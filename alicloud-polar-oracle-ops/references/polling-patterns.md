# Polling Patterns — PolarDB Oracle (`aliyun polardb-io`)
...
```bash
for i in $(seq 1 {{max_retries}}); do
  RESULT=$(aliyun polardb-io DescribeDBClusterAttribute \
...
| StartDBCluster | DescribeDBClusterAttribute | `{{user.db_cluster_id}}` | `Running` | 10s | 30 |
