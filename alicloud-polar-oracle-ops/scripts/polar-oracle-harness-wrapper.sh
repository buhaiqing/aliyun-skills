#!/bin/bash
# Polar-Oracle Runtime Harness wrapper
set -uo pipefail
...
# inject --DBType so this Oracle-focused wrapper filters the correct engine.
...
if [ "$SKILLOPT_LOADED" = true ]; then
    echo "[Wrapper] alicloud-polar-oracle-ops/scripts/polar-oracle-harness-wrapper.sh ${SUBCMD}" >&2
