...
def test_ac12_test_coverage():
    """Check coverage of scripts/lib/ is ≥ 80% (requires pytest-cov)."""
    try:
        import pytest_cov  # noqa: F401
    except ImportError:
        pytest.skip("pytest-cov not installed, skipping coverage check")

    result = subprocess.run(
        [sys.executable, "-m", "pytest", "alicloud-topo-discovery/tests/",
         "--cov=scripts.lib", "--cov-report=term-missing", "-q"],
    ...
    if cov_match:
        ...
        print(f"[DIAG] Coverage: {coverage}%")
        assert coverage >= 80, f"Coverage {coverage}% < 80%"
    else:
        # Try another format
        cov_match = re.search(r"(\d+)%", result.stdout.split("TOTAL")[-1]) if "TOTAL" in result.stdout else None
        if cov_match:
            coverage = int(cov_match.group(1))
            assert coverage >= 80, f"Coverage {coverage}% < 80%"
        else:
            # If cov not working, skip coverage check (but report)
            pytest.skip("Could not determine coverage from pytest output")