...
Large-scale Mermaid (>50 resources total across all VSwitches) auto-collapses to avoid OOM.
...
DATA_DIR = os.environ.get('TOPO_TMP_DIR') or tempfile.gettempdir()
...
for i in ecs:
    ...
    bucket = vsw_map.get(vsw)
    if bucket is None:
        continue  # skip ECS without known VSwitch (avoid mutating a temp default dict)
    bucket.setdefault('ecs', []).append({
...
large_scale = total_resources > MAX_NODES_PER_VSWITCH
if large_scale:
    print(f"[INFO] Large topology ({total_resources} resources > {MAX_NODES_PER_VSWITCH}), Mermaid using aggregated view")
...
def render_ascii():
    ...
    for vid, vs in vsw_map.items():
        ...
        if not items:
            ...
            for idx, it in enumerate(items[:MAX_NODES_PER_VSWITCH]):
                last = idx == len(items) - 1 or idx == MAX_NODES_PER_VSWITCH - 1
                pfx = "│  └─ " if last else "│  ├─ "
                lines.append(resource_line(it, f"   {pfx}"))
            if len(items) > MAX_NODES_PER_VSWITCH:
                lines.append(f"   │  └─ ... ({len(items) - MAX_NODES_PER_VSWITCH} more)")
    lines.append("```")
    lines.append("")
    if len(vpcs) > 1:
...
def render_mermaid():
    ...
    for vid, vs in vsw_map.items():
        ...
        if large_scale and len(items) > MAX_NODES_PER_VSWITCH:
            ...
            for it in items:
                ...
                base = raw_id.replace('-', '_').replace('.', '_')[:20] or 'res'
                safe_id = f"res_{base}"
                # Disambiguate collisions within this VSwitch via running counter
                collision_count[0] = collision_count[0] + 1
                if safe_id in seen_res_ids:
                    safe_id = f"{safe_id}_{collision_count[0]}"
                seen_res_ids.add(safe_id)
                label = f"{get_health(it.get('id',''),'✅')} {it['name']}"
                if 'ip' in it:
                    label += f"\\n{it['ip']}"
                lines.append(f"        {safe_id}[{label}]")
            if not items:
                safe_empty = f"empty_spot_{vid.replace('-','_')[:20]}"
                lines.append(f"        {safe_empty}[(\"(预留)\")]")
        lines.append("    end")

    # EIP → SLB connections
    # EIP IDs are unique by IP address; SLB IDs may collide with the VSwitch-loop IDs above,
    # so we append a running suffix if needed.
    for l in slbs:
        slb_id = l.get('LoadBalancerId', '')
        slb_ip = l.get('EipAddress', {}).get('IpAddress', '')
        if slb_ip:
            safe_eip = f"eip_{slb_ip.replace('.','_')}"
            base = slb_id.replace('-', '_')[:20] or l.get('LoadBalancerName', '')[:20]
            safe_slb = f"res_{base}"
            if safe_slb in seen_res_ids:
                # The corresponding node already has a numbered suffix from the VSwitch loop.
                lines.append(f"    {safe_eip}((\"{slb_ip}\")) --> {safe_slb}_eip")
            else:
