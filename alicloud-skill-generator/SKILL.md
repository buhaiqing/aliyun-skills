...
- [ ] **[GCL] Prompt templates present (when `required` or `recommended`):** `references/prompt-templates.md` exists with Generator template (hard rules list referencing rubric's per-op sub-rules) AND Critic template (independent re-query pattern, regex application, **Critic MUST NOT see `{{user.request}}`** for rubber-stamping prevention per `AGENTS.md` §12.2; **test accuracy / regression assessment** per [`docs/gcl-critic-test-assessment-block.md`](../docs/gcl-critic-test-assessment-block.md)) per [gcl-rollout-spec.md](references/gcl-rollout-spec.md) §5
...
- [ ] **[GCL] Wrapper Compliance declared (per AGENTS.md §15.8):** Generated skill MUST ship `scripts/<product>-harness-wrapper.sh` (canonical Runtime Harness wrapper; legacy `scripts/<product>-skillopt-wrapper.sh` accepted) AND declare wrapper-first in `SKILL.md` (`EXECUTION MANDATORY RULE` block referencing both `harness-wrapper` and `skillopt-wrapper`). `references/rubric.md` includes §2.4 Wrapper Compliance dimension with the bypass detection rule and exit-code 6 (`WRAPPER_BYPASS`); `references/prompt-templates.md` Generator template includes the wrapper-first rule ("all `aliyun <product>` calls MUST be routed through `scripts/<product>-harness-wrapper.sh`"); `SKILL.md` Runtime Rules table has a `CLI path` row marked MANDATORY pointing to the wrapper. The Critic scores `wrapper_compliance = 0` and the loop terminates with `WRAPPER_BYPASS` when the Generator invokes bare `aliyun <product>` (per [docs/gcl-spec.md §14.2.4](../docs/gcl-spec.md#1424-wrapper-compliance-h--recommended-for-all-skills-with-wrappers) and §3 Wrapper Compliance).

- [ ] **Wrapper-First static check (per AGENTS.md §15.8):** Generated skill MUST include `scripts/<product>-harness-wrapper.sh` and a `## Runtime Rules` section with a `CLI path` row marked MANDATORY pointing to that wrapper, plus a `Fallback` row describing when bare `aliyun <product>` is acceptable. Run `bash scripts/validate-wrapper-first-docs.sh` after generation; every product skill (excluding `alicloud-runtime-harness-ops` / `alicloud-skillopt-ops`) MUST PASS — i.e. declare wrapper-first AND ship `*-harness-wrapper.sh`. This is the static counterpart to the runtime P0 + P1 defenses (AGENTS.md §15.8 + `require_skillopt_wrapper` + Langfuse bypass alert). Template for the required table is in [alicloud-skill-template.md](references/alicloud-skill-template.md#runtime-rules-mandatory).
