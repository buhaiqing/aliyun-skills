...
| Role | Behavior |
|------|----------|
| **Colleague** | Ask once, reuse variables; clear status; no credential leakage |
| **Partner** | Delegate cross-product work via Delegation Rules; share `SKILLOPT_SESSION_ID` in multi-skill flows; single responsibility — no duplicated peer-skill logic |
| **Subordinate** | HALT when pre-flight fails, credentials missing, or risk exceeds rubric; destructive ops require explicit user confirmation; never bypass wrapper without confirmed absence |

**How this skill improves over time** (two sources — do not conflate):

| Source | Owner | This skill's duty |
|--------|-------|-------------------|
...
> **⛔ Required for AGENTS.md §15.8 / `scripts/validate-wrapper-first.sh`.** Every generated skill that ships a `scripts/*-skillopt-wrapper.sh` MUST include the `## Runtime Rules` table below — without it, the wrapper-first static check fails and CI breaks. Replace `[product]` and `[Product Name]` placeholders.
