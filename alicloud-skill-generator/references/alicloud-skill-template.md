...
| `QuotaExceeded` / `InstanceQuota不足` | 0 | — | HALT |
| `InsufficientBalance` / `余额不足` | 0 | — | HALT |
| `ResourceAlreadyExists` / `InstanceAlreadyExists` | 0 | — | Ask reuse vs new name |
| Throttling / 429 / `Throttling` | 3 | exponential | Back off; respect `Retry-After` if present |
| `InternalError` / 5xx | 3 | 2s, 4s, 8s | Retry; then HALT with correlation id (RequestId) if any |
