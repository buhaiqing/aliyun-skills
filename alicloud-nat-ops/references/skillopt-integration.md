# Runtime Harness Integration for alicloud-nat-ops
...
3. **Resource Not Found** (`NatGatewayNotFound`) — verifies resource existence
...
### Direct Command
```bash
aliyun vpc DescribeInstances --skillopt-enable --RegionId cn-hangzhou
```

### Wrapper Script
```bash
cd $(pwd)/alicloud-nat-ops
...
./scripts/nat-harness-wrapper.sh DescribeNatGateways --RegionId cn-hangzhou

# Check local trace
ls -lt .runtime/traces/alicloud-nat-ops/ | head -3

# Verify in Langfuse UI
# Navigate to https://hai-langfuse-int.hd123.com → Sessions → skill:alicloud-nat-ops
```
