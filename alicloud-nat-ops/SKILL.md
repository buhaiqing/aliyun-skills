...
> 以下所有代码块中的 `aliyun nat ...` 命令在执行时应替换为 `./scripts/vpc-harness-wrapper.sh <subcommand> ...`。
> 仅在 wrapper 脚本不可用时，才退回到原生 `aliyun nat` CLI 调用。
