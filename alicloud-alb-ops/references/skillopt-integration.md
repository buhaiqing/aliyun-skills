# Runtime Harness Integration for alicloud-alb-ops
...
3. **Resource Not Found** (`LoadBalancerNotFound`) — verifies resource existence
...
### Direct Command
```bash
aliyun alb DescribeInstances --skillopt-enable --RegionId cn-hangzhou
```
...
./scripts/alb-harness-wrapper.sh ListLoadBalancers --RegionId cn-hangzhou

# Check local trace
ls -lt .runtime/traces/alicloud-alb-ops/ | head -3

# Verify in Langfuse UI
# Navigate to https://hai-langfuse-int.hd123.com → Sessions → skill:alicloud-alb-ops
```
