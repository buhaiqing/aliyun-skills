#!/bin/bash
# SkillOpt Core Library for alicloud-alb-ops
# Self-repair and dynamic optimization for ALB CLI commands.
...
SKILLOPT_LOG_LABEL="ALB-SkillOpt"
SKILLOPT_SKILL_TAG="alicloud-alb-ops"
...
SKILLOPT_LOG_FILE="${ALIBABA_CLOUD_LOG_DIR:-${_SKILLOPT_LOGS_DIR:-$_SKILLOPT_RUNTIME_ROOT}}/alb-skillopt-$(date +%Y%m%d).log"
SKILLOPT_RUNTIME_DATA="${_SKILLOPT_METRICS_DATA_DIR:-$_SKILLOPT_RUNTIME_ROOT}/alb-skillopt-runtime.json"
# --- End shared core ---
    ...
    case "$error_code" in
        ...
        LoadBalancerNotFound|ResourceNotFound)
            ...
            if [[ -n "$instance_id" && -n "$region" ]]; then
                skillopt_log "repair[LoadBalancerNotFound|ResourceNotFound]: checking $instance_id in $region"
                if aliyun ecs DescribeInstances \
                        --RegionId "$region" \
                        --InstanceIds "[\"$instance_id\"]" >/dev/null 2>&1; then
                    skillopt_log "repair[LoadBalancerNotFound|ResourceNotFound]: instance exists, retrying"
                    if skillopt_run_aliyun "$product" "$action" "${params[@]}"; then
                        repair_failed=0
                    fi
                else
                    skillopt_log "repair[LoadBalancerNotFound|ResourceNotFound]: instance $instance_id not found in $region"
                fi
            else
                skillopt_log "repair[LoadBalancerNotFound|ResourceNotFound]: cannot extract instanceId/region, skipping"
            fi
            ;;

        Forbidden|NoPermission)
            skillopt_log "repair[Forbidden]: RAM policy suggestion"
            skillopt_log "HINT: Ensure AK has policy: {\"Action\":[\"alb:*\"],\"Effect\":\"Allow\",\"Resource\":\"*\"}"
            ;;
        ...
        QuotaExceeded)
            skillopt_log "Quota exceeded for ALB"
            ;;
