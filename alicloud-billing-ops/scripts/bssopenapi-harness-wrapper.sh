#!/bin/bash
# Billing (bssopenapi) Runtime Harness wrapper
set -uo pipefail
...
PRODUCT="bssopenapi"
if [[ ${#} -gt 0 && ("$1" == "bssopenapi" || "$1" == "bssopenapi2") ]]; then
    PRODUCT="$1"
    shift
fi
...
if [ "$SKILLOPT_LOADED" = true ]; then
    echo "[Wrapper] alicloud-billing-ops/scripts/bssopenapi-harness-wrapper.sh ${SUBCMD}" >&2
