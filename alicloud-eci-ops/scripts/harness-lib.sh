#!/bin/bash
# SkillOpt product overlay for alicloud-eci-ops
# Self-repair overlay for ECI CLI commands.
...
SKILLOPT_LOG_LABEL="ECI-SkillOpt"
SKILLOPT_SKILL_TAG="alicloud-eci-ops"
...
SKILLOPT_LOG_FILE="${ALIBABA_CLOUD_LOG_DIR:-${_SKILLOPT_LOGS_DIR:-$_SKILLOPT_RUNTIME_ROOT}}/eci-skillopt-$(date +%Y%m%d).log"
SKILLOPT_RUNTIME_DATA="${_SKILLOPT_METRICS_DATA_DIR:-$_SKILLOPT_RUNTIME_ROOT}/eci-skillopt-runtime.json"
# --- End shared core ---
    ...
    case "$error_code" in
        ...
        InvalidParameter|InvalidJSON|MissingParameter|MissingParameter.RegionId|InvalidParameter.RegionId|MissingRegionId|DataRetentionExceeded|InvalidTimeRange|TimeRangeExceeded|InvalidParameter.StartTime|InvalidParameter.TimeRange)
            ...
            if $is_region_error; then
                ...
                if [[ -z "$region" ]]; then
                    if _SKILLOPT_INTERNAL_REPAIR=1 skillopt_run_aliyun eci DescribeRegions --output cols=RegionId rows=Regions.Region 2>/dev/null; then
