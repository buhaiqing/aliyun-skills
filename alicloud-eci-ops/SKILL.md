...
description: >-
  ...
  `alicloud-ask-ops`. Does NOT handle ECS instances, ACK
...
```bash
# 通用轮询，参数见 [references/polling-patterns.md](references/polling-patterns.md)（CreateContainerGroup → Running/Succeeded/Failed, 60×5s）
```
...
| **K8s 集群内 ECI (ASK Pod)** | [`alicloud-ask-ops`](../alicloud-ask-ops/SKILL.md) | 集群 ID、Pod 名 — **不要同时用本 skill 管理同一个 Pod** |
...
- [Polling Patterns](references/polling-patterns.md) — ContainerGroup 状态/删除轮询模板
