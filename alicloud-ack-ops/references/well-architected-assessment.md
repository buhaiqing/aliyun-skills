# Well-Architected Assessment — Alibaba Cloud ACK

This document evaluates the ACK skill's operations against Alibaba Cloud's
...
|-------------|----------|
| **RAM Policy** | Use scoped permissions: `acs:ack:*:*:cluster/node/namespace/*`. Avoid wildcard `*` actions. |
...
| Subscription (Worker Nodes) | Use Case | Savings |
...
|--------|-----------|---------------------|
| CPUUtilization | acs_alicloud_ack | Threshold |
| MemoryUsage | acs_alicloud_ack | Threshold |
| PodCount | acs_alicloud_ack | Threshold |
| APIServerLatency | acs_alicloud_ack | Threshold |
