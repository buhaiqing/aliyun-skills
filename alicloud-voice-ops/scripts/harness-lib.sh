#!/bin/bash
# SkillOpt Core Library for alicloud-voice-ops
# Self-repair and dynamic optimization for Voice (dyvmsapi) CLI commands.
...
SKILLOPT_LOG_LABEL="Voice-SkillOpt"
SKILLOPT_SKILL_TAG="alicloud-voice-ops"
...
SKILLOPT_LOG_FILE="${ALIBABA_CLOUD_LOG_DIR:-${_SKILLOPT_LOGS_DIR:-$_SKILLOPT_RUNTIME_ROOT}}/voice-skillopt-$(date +%Y%m%d).log"
SKILLOPT_RUNTIME_DATA="${_SKILLOPT_METRICS_DATA_DIR:-$_SKILLOPT_RUNTIME_ROOT}/voice-skillopt-runtime.json"
# --- End shared core ---
...
skillopt_repair_error() {
    ...
    case "$error_code" in
        ...
        Forbidden|NoPermission)
            ...
            skillopt_log "HINT: Ensure AK has policy: {\"Action\":[\"cms:*\"],\"Effect\":\"Allow\",\"Resource\":\"*\"}"
            ;;
