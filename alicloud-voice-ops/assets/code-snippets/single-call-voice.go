...
// Operation: SingleCallByVoice — send single voice notification using pre-recorded audio file
// Usage:
//   cd alicloud-voice-ops/assets/code-snippets
//   CALLED_NUMBER=13800138000 VOICE_CODE=123456 SHOW_NUMBER=4008123123 go run single-call-voice.go
...
func main() {
	...
	// Build request
	req := &dyvmsapi.SingleCallByVoiceRequest{
	...
	// Execute
	resp, err := client.SingleCallByVoice(req)
	if err != nil {
		log.Fatalf("SingleCallByVoice failed: %v", err)
	}
