# Runtime Harness Integration for alicloud-fc-ops
...
### Direct Command
```bash
aliyun fc DescribeInstances --skillopt-enable --RegionId cn-hangzhou
```

### Wrapper Script
```bash
cd $(pwd)/alicloud-fc-ops
./scripts/fc-skillopt-wrapper.sh DescribeInstances --RegionId cn-hangzhou
```
...
./scripts/fc-harness-wrapper.sh GET /2023-03-30/functions

# Check local trace
ls -lt .runtime/traces/alicloud-fc-ops/ | head -3

# Verify in Langfuse UI
# Navigate to https://hai-langfuse-int.hd123.com → Sessions → skill:alicloud-fc-ops
```
