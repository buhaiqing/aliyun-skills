# Runtime Harness Integration for alicloud-elasticsearch-ops
...
3. **Resource Not Found** (`InstanceNotFound`) — verifies resource existence
...
### Direct Command
```bash
aliyun elasticsearch DescribeInstances --skillopt-enable --RegionId cn-hangzhou
```

### Wrapper Script
```bash
cd $(pwd)/alicloud-elasticsearch-ops
./scripts/elasticsearch-harness-wrapper.sh DescribeInstances --RegionId cn-hangzhou
```
...
./scripts/elasticsearch-harness-wrapper.sh ListInstance --RegionId cn-hangzhou

# Check local trace
ls -lt .runtime/traces/alicloud-elasticsearch-ops/ | head -3

# Verify in Langfuse UI
# Navigate to https://hai-langfuse-int.hd123.com → Sessions → skill:alicloud-elasticsearch-ops
```
