#!/bin/bash
# SkillOpt Core Library for alicloud-elasticsearch-ops
# Self-repair and dynamic optimization for Elasticsearch operations.
# This skill is SDK-only (no native aliyun CLI); uses Go JIT SDK scripts.
...
SKILLOPT_LOG_LABEL="Elasticsearch-SkillOpt"
SKILLOPT_SKILL_TAG="alicloud-elasticsearch-ops"
...
SKILLOPT_LOG_FILE="${ALIBABA_CLOUD_LOG_DIR:-${_SKILLOPT_LOGS_DIR:-$_SKILLOPT_RUNTIME_ROOT}}/elasticsearch-skillopt-$(date +%Y%m%d).log"
SKILLOPT_RUNTIME_DATA="${_SKILLOPT_METRICS_DATA_DIR:-$_SKILLOPT_RUNTIME_ROOT}/elasticsearch-skillopt-runtime.json"
# --- End shared core ---
